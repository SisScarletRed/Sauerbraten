...
struct iteminfo
{
	...
	char *name;
	iteminfo() {}
	iteminfo(selinfo s, const char *name_ = "")
	{
		sel = s;
		trans = s;
	...
	void addlink(selinfo s, matrix3x3 t = matrix3x3(vec(1, 0, 0), vec(0, 1, 0), vec(0, 0, 1)))
	{
	    links.add(linkinfo(this, s, t));
    }
	void clearlinks() { links.setsize(0); }
...
extern void mpcopy(editinfo *&e, selinfo &sel, bool local, vector <linkinfo> &linkcopys, int &linkcopygrid);
extern void mppaste(editinfo *&e, selinfo &sel, bool local, vector <linkinfo> &linkcopys, int &linkcopygrid);
...
// command
extern int debugquality;
extern int variable(const char *name, int min, int cur, int max, int *storage, identfun fun, int flags);
extern int variable(const char *name, const char *description, int min, int cur, int max, int *storage, identfun fun, int flags);
extern float fvariable(const char *name, float min, float cur, float max, float *storage, identfun fun, int flags);
extern float fvariable(const char *name, const char *description, float min, float cur, float max, float *storage, identfun fun, int flags);
extern char *svariable(const char *name, const char *cur, char **storage, identfun fun, int flags);
extern char *svariable(const char *name, const char *description, const char *cur, char **storage, identfun fun, int flags);
...
extern bool addcommand(const char *name, const char *description, const char *params, identfun fun, const char *narg);
...
extern void draw_textfa(const char *fstr, int a, int left, int top, ...) PRINTFARGS(1, 5);
...
extern void removedamagescreen();
extern void damageblend(int n, bool lowhp = false);
...
enum
{
    ...
    PART_EXPLOSION, PART_EXPLOSION_BLUE, PART_EXPLOSION_PLASMA,
...
extern void particle_trail(int type, int fade, const vec &from, const vec &to, int color = 0xFFFFFF, float size = 1.0f, int gravity = 20, bool boobles = false);
...
extern void particle_health(const vec &s, float val, int type, int fade, int color, int color2, float size);
...
struct g3d_gui
{
    ...
    virtual int text(const char *text, int color, const char *icon = NULL, const char *desc = NULL) = 0;
    int textf(const char *fmt, int color, const char *icon = NULL, const char *desc = NULL, ...) PRINTFARGS(2, 6)
    {
        defvformatstring(str, desc, fmt);
        return text(str, color, icon, desc);
    }
    virtual int button(const char *text, int color, const char *icon = NULL, const char *desc = NULL) = 0;
	int buttonf(const char *fmt, int color, const char *icon = NULL, const char *desc = NULL, ...) PRINTFARGS(2, 6)
    {
        defvformatstring(str, desc, fmt);
        return button(str, color, icon, desc);
    }
    virtual int title(const char *text, int color, const char *icon = NULL, const char *desc = NULL) = 0;
    int titlef(const char *fmt, int color, const char *icon = NULL, const char *desc = NULL, ...) PRINTFARGS(2, 6)
    {
        defvformatstring(str, desc, fmt);
        return title(str, color, icon, desc);
    }
    ...
    virtual int textwithtextureicon(const char *text, int color, const char *icon,
                                    ...
                                    float tx, float ty, float tszx, float tszy) = 0;
