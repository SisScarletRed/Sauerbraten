...
struct ident
{
    ...
    ident(int t, const char *n, int m, int x, int *s, void *f = NULL, int flags = 0)
        : type(t), flags(flags | (m > x ? IDF_READONLY : 0)), name(n), desc("no help available"), params(""), minval(m), maxval(x), fun((identfun)f)
    { storage.i = s; }
    ident(int t, const char *n, const char *d, const char *p, int m, int x, int *s, void *f = NULL, int flags = 0)
        : type(t), flags(flags | (m > x ? IDF_READONLY : 0)), name(n), desc(d), params(p), minval(m), maxval(x), fun((identfun)f)
    { storage.i = s; }

    // ID_FVAR
    ident(int t, const char *n, float m, float x, float *s, void *f = NULL, int flags = 0)
        : type(t), flags(flags | (m > x ? IDF_READONLY : 0)), name(n), desc("no help available"), params(""), minvalf(m), maxvalf(x), fun((identfun)f)
    { storage.f = s; }
    ident(int t, const char *n, const char *d, const char *p, float m, float x, float *s, void *f = NULL, int flags = 0)
        : type(t), flags(flags | (m > x ? IDF_READONLY : 0)), name(n), desc(d), params(p), minvalf(m), maxvalf(x), fun((identfun)f)
    { storage.f = s; }

    // ID_SVAR
    ident(int t, const char *n, char **s, void *f = NULL, int flags = 0)
        : type(t), flags(flags), name(n), desc("no help available"), params(""), fun((identfun)f)
    { storage.s = s; }
    ident(int t, const char *n, const char *d, const char *p, char **s, void *f = NULL, int flags = 0)
        : type(t), flags(flags), name(n), desc(d), params(p), fun((identfun)f)
    { storage.s = s; }

    // ID_ALIAS
    ident(int t, const char *n, char *a, int flags)
        : type(t), valtype(VAL_STR), flags(flags), name(n), desc("no help available"), params(""), code(NULL), stack(NULL)
    { val.s = a; }
    ident(int t, const char *n, int a, int flags)
        : type(t), valtype(VAL_INT), flags(flags), name(n), desc("no help available"), params(""), code(NULL), stack(NULL)
    { val.i = a; }
    ident(int t, const char *n, float a, int flags)
        : type(t), valtype(VAL_FLOAT), flags(flags), name(n), desc("no help available"), params(""), code(NULL), stack(NULL)
    { val.f = a; }
    ident(int t, const char *n, int flags)
        : type(t), valtype(VAL_NULL), flags(flags), name(n), desc("no help available"), params(""), code(NULL), stack(NULL)
    {}
    ident(int t, const char *n, const tagval &v, int flags)
        : type(t), valtype(v.type), flags(flags), name(n), desc("no help available"), params(""), code(NULL), stack(NULL)
    { val = v; }

    // ID_COMMAND
    ident(int t, const char *n, const char *args, uint argmask, int numargs, void *f = NULL, int flags = 0)
        : type(t), numargs(numargs), flags(flags), name(n), desc("no help available"), params(""), args(args), argmask(argmask), fun((identfun)f)
    {}
    ident(int t, const char *n, const char *d, const char *p, const char *args, uint argmask, int numargs, void *f = NULL, int flags = 0)
        : type(t), numargs(numargs), flags(flags), name(n), desc(d), params(p), args(args), argmask(argmask), fun((identfun)f)
    {}
...
#define RVAR(name, cur) _VAR(name, name, -INT_MAX, cur, INT_MAX, IDF_READONLY)
#define RVAR0(name) _VAR(name, name, -INT_MAX, 0, INT_MAX, IDF_READONLY)
...
#define QCOMMANDN(name, description, params, fun, nargs) UNUSED static bool __dummy_##fun = addcommand(#name, description, params, (identfun)fun, nargs)
#define QCOMMAND(name, description, params, nargs) QCOMMANDN(name, description, params, name, nargs)

// INTEGER (int)
#define _QVAR(name, description, global, min, cur, max, persist) int global = variable(#name, description, min, cur, max, &global, NULL, persist)
#define QVARN(name, description, global, min, cur, max) _QVAR(name, description, global, min, cur, max, 0)
#define QVARNP(name, description, global, min, cur, max) _QVAR(name, description, global, min, cur, max, IDF_QUALITY)
#define QVAR(name, description, min, cur, max) _QVAR(name, description, name, min, cur, max, 0)
#define QVARP(name, description, min, cur, max) _QVAR(name, description, name, min, cur, max, IDF_QUALITY)
#define QRVAR(name, description, cur) _QVAR(name, description, name, -INT_MAX, cur, INT_MAX, IDF_READONLY|IDF_QUALITY)
#define QRVAR0(name, description) _QVAR(name, description, name, -INT_MAX, 0, INT_MAX, IDF_READONLY|IDF_QUALITY)

#define _QVARF(name, description, global, min, cur, max, body, persist)  void var_##name(); int global = variable(#name, description, min, cur, max, &global, var_##name, persist); void var_##name() { body; }
#define QVARFN(name, description, global, min, cur, max, body) _QVARF(name, description, global, min, cur, max, body, 0)
#define QVARF(name, description, min, cur, max, body) _QVARF(name, description, name, min, cur, max, body, 0)
#define QVARFP(name, description, min, cur, max, body) _QVARF(name, description, name, min, cur, max, body, IDF_QUALITY)

// HEXA (int)
#define _QHVAR(name, description, global, min, cur, max, persist)  int global = variable(#name, description, min, cur, max, &global, NULL, persist | IDF_HEX)
#define QHVARN(name, description, global, min, cur, max) _QHVAR(name, description, global, min, cur, max, 0)
#define QHVARNP(name, description, global, min, cur, max) _QHVAR(name, description, global, min, cur, max, IDF_QUALITY)
#define QHVAR(name, description, min, cur, max) _QHVAR(name, description, name, min, cur, max, 0)
#define QHVARP(name, description, min, cur, max) _QHVAR(name, description, name, min, cur, max, IDF_QUALITY)

#define _QHVARF(name, description, global, min, cur, max, body, persist)  void var_##name(); int global = variable(#name, description, min, cur, max, &global, var_##name, persist | IDF_HEX); void var_##name() { body; }
#define QHVARFN(name, description, global, min, cur, max, body) _QHVARF(name, description, global, min, cur, max, body, 0)
#define QHVARF(name, description, min, cur, max, body) _QHVARF(name, description, name, min, cur, max, body, 0)
#define QHVARFP(name, description, min, cur, max, body) _QHVARF(name, description, name, min, cur, max, body, IDF_QUALITY)

// FLOAT (float)
#define _QFVAR(name, description, global, min, cur, max, persist) float global = fvariable(#name, description, min, cur, max, &global, NULL, persist)
#define QFVARN(name, description, global, min, cur, max) _QFVAR(name, description, global, min, cur, max, 0)
#define QFVARNP(name, description, global, min, cur, max) _QFVAR(name, description, global, min, cur, max, IDF_QUALITY)
#define QFVAR(name, description, min, cur, max) _QFVAR(name, description, name, min, cur, max, 0)
#define QFVARP(name, description, min, cur, max) _QFVAR(name, description, name, min, cur, max, IDF_QUALITY)

#define _QFVARF(name, description, global, min, cur, max, body, persist) void var_##name(); float global = fvariable(#name, description, min, cur, max, &global, var_##name, persist); void var_##name() { body; }
#define QFVARFN(name, description, global, min, cur, max, body) _QFVARF(name, description, global, min, cur, max, body, 0)
#define QFVARF(name, description, min, cur, max, body) _QFVARF(name, description, name, min, cur, max, body, 0)
#define QFVARFP(name, description, min, cur, max, body) _QFVARF(name, description, name, min, cur, max, body, IDF_QUALITY)

// STRING (const char *)
#define _QSVAR(name, description, global, cur, persist) char *global = svariable(#name, description, cur, &global, NULL, persist)
#define QSVARN(name, description, global, cur) _QSVAR(name, description, global, cur, 0)
#define QSVARNP(name, description, global, cur) _QSVAR(name, description, global, cur, IDF_QUALITY)
#define QSVAR(name, description, cur) _QSVAR(name, description, name, cur, 0)
#define QSVARP(name, description, cur) _QSVAR(name, description, name, cur, IDF_QUALITY)

#define _QSVARF(name, description, global, cur, body, persist) void var_##name(); char *global = svariable(#name, description, cur, &global, var_##name, persist); void var_##name() { body; }
#define QSVARFN(name, description, global, cur, body) _QSVARF(name, description, global, cur, body, 0)
#define QSVARF(name, description, cur, body) _QSVARF(name, description, name, cur, body, 0)
#define QSVARFP(name, description, cur, body) _QSVARF(name, description, name, cur, body, IDF_QUALITY)

// COMMANDS
#define QICOMMANDNS(name, description, params, cmdname, nargs, proto, b) template<int N> struct cmdname; template<> struct cmdname<__LINE__> { static bool init; static void run proto; }; bool cmdname<__LINE__>::init = addcommand(name, description, params, (identfun)cmdname<__LINE__>::run, nargs); void cmdname<__LINE__>::run proto \
    { b; }
#define QICOMMANDN(name, description, params, cmdname, nargs, proto, b) QICOMMANDNS(#name, description, params, cmdname, nargs, proto, b)
#define QICOMMANDNAME(name) _icmd_##name
#define QICOMMAND(name, description, params, nargs, proto, b) QICOMMANDN(name, description, params, QICOMMANDNAME(name), nargs, proto, b)
#define QICOMMANDSNAME _icmds_
#define QICOMMANDS(name, description, params, nargs, proto, b) QICOMMANDNS(name, description, params, QICOMMANDSNAME, nargs, proto, b)
