...
char *getrelativefilename(const char *currentdir, const char *absolutefilename)
{
    int afmarker = 0, rfmarker = 0,
        cdLen = 0, afLen = 0,
        i = 0,
        levels = 0;
    static string relfilename;
    cdLen = strlen(currentdir);
    afLen = strlen(absolutefilename);

    while(i < afLen && i < cdLen && currentdir[i] == absolutefilename[i]) i++;
    if(i == cdLen && (absolutefilename[i] == PATHDIV || absolutefilename[i - 1] == PATHDIV))
    {
        if(absolutefilename[i] == PATHDIV) i++;
        strcpy(relfilename, &absolutefilename[i]);
        return relfilename;
    }
    afmarker = i;
    levels = 1;
    ...
    {
        i++;
        if(currentdir[i] == PATHDIV)
        {
            i++;
            if(currentdir[i] != '\0') levels++;
        }
    }
    while(afmarker > 0 && absolutefilename[afmarker - 1] != PATHDIV) afmarker--;
    rfmarker = 0;
    for(i = 0; i < levels; i++)
    {
        relfilename[rfmarker++] = '.';
        relfilename[rfmarker++] = '.';
        relfilename[rfmarker++] = PATHDIV;
    }
    strcpy(&relfilename[rfmarker], &absolutefilename[afmarker]);
    return relfilename;
}
...
char *cutdir(char *filename)
{
    ...
    loopi(rarelen) filename[i] = p[i+1];
    return filename;
}
...
const char *findfile(const char *filename, const char *mode, bool withhomedir)
{
    static string s;
    if(homedir[0])
    {
        formatstring(s)("%s%s", withhomedir ? homedir : "", filename);
