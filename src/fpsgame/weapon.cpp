...
namespace game
{
    ...
    ICOMMAND(getweaponname, "", (), result(guns[player1->gunselect].name));
    ...
    struct bouncer : physent
    {
        ...
        bouncer() : bounces(0), roll(0), inwater(0), variant(0)
        {
            type = ENT_BOUNCE;
        }
    };
    ...
    VARP(coloredflares, 0, 0, 1);

    HVARP(glflarecolor, 0, 0x404040, 0xFFFFFF);
    FVARP(glflaresize, 0, 2.4f, 5.0f);
    VARP(gltrailtime, 0, 150, 1000);
        ...
        loopv(bouncers)
        {
            ...
            if(bnc.bouncetype==BNC_GRENADE && bnc.vel.magnitude() > 50.0f)
            {
                ...
                if(lookupmaterial(pos)==MAT_WATER)
                    ...
                    regular_particle_splash(PART_SMOKE, 1, 150, pos, coloredflares ? glflarecolor : 0x404040, glflaresize, 50, -20);
            }
            ...
            if(bnc.bouncetype==BNC_GIBS && bnc.vel.magnitude() > 10.0f)
            {
                vec pos(bnc.o);
                pos.add(vec(bnc.offset).mul(bnc.offsetmillis/float(OFFSETMILLIS)));
                regular_particle_splash(PART_BLOOD, 1, 9999, pos, 0x60FFFF  , 1.0f, 50);
            }
            vec old(bnc.o);
            bool stopped = false;
            ...
            if(stopped)
            {
                if(bnc.bouncetype==BNC_GRENADE)
                {
                    ...
                    if(bnc.local)
                    {
                        ...
                        demo::explodefx(bnc.owner->clientnum, GUN_GL, bnc.id-maptime);
                    }
                }
    ...
    void damageeffect(int damage, fpsent *d, bool thirdperson)
    {
        ...
        if(blood)
        {
            if(damage>25) particle_splash(PART_BLOOD, damage/10, 1000, p, 0x60FFFF, 2.96f);
            if(damage>100)
            {
                if(lookupmaterial(camera1->o)!=MAT_WATER && lookupmaterial(d->o)!=MAT_WATER)
                    playsound(S_GIB1+rnd(3), &d->o);
            }
        }

        if(thirdperson)
            particle_textcopy(d->abovehead(), tempformatstring("%d", damage), PART_TEXT, 2000, 0xFF4B19, 4.0f, -12);
    }
    ...
    void hit(int damage, dynent *d, fpsent *at, const vec &vel, int gun, float info1, int info2 = 1)
    {
        ...
        else
        {
            ...
            else if(gun==GUN_FIST) playsound(S_SCH1+rnd(3), &f->o);
        }
    }
    ...
    void explode(bool local, fpsent *owner, const vec &v, dynent *safe, int damage, int gun)
    {
        ...
        if(expsmoke)
        {
            particle_splash(PART_SMOKE, 5, 2500, v, 0x222222, 12.0f, 50, 500, NULL, 1, false, 3);
            particle_splash(PART_SMOKE, 5, 200, v, 0x222222, 12.0f, 50, 500, NULL, 1, false, 4);
        }

        if(gun==GUN_RL)
        {
            particle_splash(PART_FLAME1, 15, 30, v, 0xFFFFFF, 3.2f, 300, NULL, NULL, NULL, true);
            particle_splash(PART_FLAME2, 15, 30, v, 0xFFFFFF, 3.2f, 300, NULL, NULL, NULL, true);
            particle_splash(PART_FLAME2, 15, 30, v, 0xFFFFFF, 3.2f, 300, NULL, NULL, NULL, true);
            ...
            particle_fireball(v, RL_DAMRAD, PART_EXPLOSION, 100, 0xFF8080, 4.0f);
        }
        else if(gun==GUN_GL)
        {
        ...
        if(lookupmaterial(camera1->o)==MAT_WATER || lookupmaterial(v)==MAT_WATER)
            playsound(S_UWEN, &v);
        else playsound(gun==GUN_RL?S_RLHIT1+rnd(3):S_RLEX1+rnd(3), &v);

        vec occlusioncheck;
        if(gun==GUN_GL && raycubelos(v, camera1->o, occlusioncheck))
            particle_flare(v, v, 100, PART_GLOW, 0x444477, 10.0f, NULL, 5);

        if(gun==GUN_RL) adddynlight(v, 1.15f*RL_DAMRAD, vec(2, 1.5f, 1), 100, 100, 0, RL_DAMRAD/2, vec(1, 0.75f, 0.5f));
        else if(gun==GUN_GL) adddynlight(v, 1.15f*RL_DAMRAD, vec(2, 1.5f, 1), 100, 100, 0, 8, vec(0.25f, 1, 1));
        else adddynlight(v, 1.15f*RL_DAMRAD, vec(2, 1.5f, 1), 100, 100);
    ...
    void projsplash(projectile &p, vec &v, dynent *safe, int damage)
    {
        if(guns[p.gun].part)
        {
            particle_splash(PART_SPARK, 100, 200, v, 0xFFFFF, 0.1f);
    ...
    HVARP(rlflarecolor, 0, 0x404040, 0xFFFFFF);
    FVARP(rlflaresize, 0, 2.4f, 5.0f);
    VARP(rltrailtime, 0, 300, 1000);
        ...
        loopv(projs)
        {
            ...
            if(!exploded)
            {
                ...
                else
                {
                    ...
                    else if(lookupmaterial(pos)!=MAT_WATER)
                    {
                        if(p.gun==GUN_RL) regular_particle_splash(PART_SMOKE, 2, rltrailtime, pos, coloredflares ? rlflarecolor : 0x404040, rlflaresize, 12, 502);
                        else regular_particle_splash(PART_SMOKE, 2, 300, pos, 0x404040, 2.4f, 12, 502);
                    }
            ...
            if(exploded)
            {
                if(p.local)
                {
                    addmsg(N_EXPLODE, "rci3iv", p.owner, lastmillis-maptime, p.gun, p.id-maptime,
                            hits.length(), hits.length()*sizeof(hitmsg)/sizeof(int), hits.getbuf());
                    demo::explodefx(p.owner->clientnum, p.gun, p.id-maptime);
                }
                projs.remove(i--);
            }
            else p.o = v;
    ...
    HVARP(sgflarecolor, 0, 0xFFC864, 0xFFFFFF);
    HVARP(cgflarecolor, 0, 0xFFC864, 0xFFFFFF);
    HVARP(riflarecolor, 0, 0x404040, 0xFFFFFF);

    FVARP(sgflaresize, 0, 0.28f, 5.0f);
    FVARP(cgflaresize, 0, 0.28f, 5.0f);
    FVARP(riflaresize, 0, 0.6f, 5.0f);

    VARP(sgtrailtime, 0, 300, 1000);
    VARP(cgtrailtime, 0, 600, 1000);
    VARP(ritrailtime, 0, 400, 1000);

    VARP(riflegravity, 0, 1, 1);
    VARP(ritrails, 0, 1, 1);
    ...
    void shoteffects(int gun, const vec &from, const vec &to, fpsent *d, bool local, int id, int prevaction)     // create visual effect from a shot
    {
        ...
        switch(gun)
        {
            ...
            case GUN_SG:
            {
                ...
                loopi(guns[gun].rays)
                {
                    ...
                    particle_trail(0, 0, from, rays[i], 0, 0, 0, true);
                    ...
                    vec buf(rays[i]);
                    buf.sub(d->muzzle);
                    buf.normalize().mul(6000.0f);
                    particle_flying_flare(d->muzzle, buf, 500, PART_BULLET, 0xFFFFFF, 0.5f, 1000);
                    if(coloredflares) particle_flare(hudgunorigin(gun, from, rays[i], d), rays[i], sgtrailtime, PART_STREAK, sgflarecolor, sgflaresize);
                    if(!local) adddecal(DECAL_BULLET, rays[i], vec(from).sub(rays[i]).normalize(), 2.0f);
                }
                loopi(3) sgnearmiss[i] = 0;
                if(muzzlelight) adddynlight(hudgunorigin(gun, d->o, to, d), 30, vec(0.5f, 0.375f, 0.25f), 100, 50, DL_FLASH, 0, vec(0, 0, 0), d);
                break;
            }
            ...
            case GUN_PISTOL:
            {
                if(lookupmaterial(camera1->o)!=MAT_WATER) playsound(S_BHIT1+rnd(3), &to, NULL, NULL, NULL, NULL, 200);
                ...
                if(d->quadmillis)
                {
                    ...
                    playsound(S_QHIT1+rnd(3), &to, NULL, NULL, NULL, NULL, 100);
                }
                ...
                buf.sub(d->muzzle);
                buf.normalize().mul(6000.0f);
                particle_flying_flare(d->muzzle, buf, 500, PART_BULLET, 0xFFFFFF, 0.5f, 100);

                particle_splash(PART_SMOKE, 3, 500, d->muzzle, 0x222222, 1.4f, 50, 501, NULL, 2, NULL, 2);
                particle_trail(1, 1, from, to, 1, 1, 1, true);

                if(coloredflares) particle_flare(hudgunorigin(gun, from, to, d), to, cgtrailtime, PART_STREAK, cgflarecolor, cgflaresize);

                if(muzzleflash && d->muzzle.x >= 0) particle_flare(d->muzzle, d->muzzle, gun==GUN_CG ? 100 : 50, PART_MUZZLE_FLASH1, 0xFFFFFF, gun==GUN_CG ? 3.375f : 1.875f, d);
                if(!local) adddecal(DECAL_BULLET, to, vec(from).sub(to).normalize(), 2.0f);
                if(muzzlelight) adddynlight(hudgunorigin(gun, d->o, to, d), gun==GUN_CG ? 30 : 15, vec(0.5f, 0.375f, 0.25f), gun==GUN_CG ? 100 : 200, gun==GUN_CG ? 50 : 100, DL_FLASH, 0, vec(0, 0, 0), d);
                break;
            }

            case GUN_RL:
                if(muzzleflash && d->muzzle.x >= 0)
                    if(coloredflares) particle_flare(d->muzzle, d->muzzle, gltrailtime, PART_MUZZLE_FLASH2, glflarecolor, glflaresize, d);
            case GUN_FIREBALL:
            case GUN_ICEBALL:
            case GUN_SLIMEBALL:
            ...
            case GUN_GL:
            {
                ...
                if(muzzlelight) adddynlight(hudgunorigin(gun, d->o, to, d), 100, vec(0.5f, 0.375f, 0.25f), 100, 100, DL_FLASH, 0, vec(0, 0, 0), d);
            ...
            case GUN_RIFLE:
                ...
                particle_splash(PART_SMOKE, 3, 500, to, 0x444444, 1.4f, 50, 504, NULL, 2, NULL, 1);
                if(d->quadmillis) particle_trail(PART_FLAME1, 50, hudgunorigin(gun, from, to, d), to, 0xFF0000, 0.6f, 200);
                if(ritrailtime && (ritrails || coloredflares)) particle_trail(PART_SMOKE, ritrailtime, hudgunorigin(gun, from, to, d), to, riflarecolor, riflaresize, (riflegravity) ? 20 : 0);
                ...
                if(!local) adddecal(DECAL_CRACK, to, vec(from).sub(to).normalize(), 8.0f);
                if(muzzlelight) adddynlight(hudgunorigin(gun, d->o, to, d), 25, vec(0.5f, 0.375f, 0.25f), 100, 75, DL_FLASH, 0, vec(0, 0, 0), d);
                break;
        }
        ...
        switch(sound)
        {
            ...
            case S_FLAUNCH:
                ...
                break;
            case S_CG:
                if(d==hudplayer()) playsound(lookupmaterial(camera1->o)!=MAT_WATER?S_CG1+rnd(3):S_UWST2);
                else
                {
                    if(lookupmaterial(camera1->o)==MAT_WATER)
                        playsound(S_UWST2, &d->o);
                ...
                break;
            case S_PISTOL:
                if(d==hudplayer()) playsound(lookupmaterial(camera1->o)!=MAT_WATER?S_PL1+rnd(3):S_UWST2);
                else
                {
                    if(lookupmaterial(camera1->o)==MAT_WATER)
                        playsound(S_UWST2, &d->o);
                ...
                break;
            case S_RLFIRE:
                if(d==hudplayer()) playsound(lookupmaterial(camera1->o)!=MAT_WATER?sound:S_UWST3);
                else
                {
                    if(lookupmaterial(camera1->o)==MAT_WATER)
                        playsound(S_UWST3, &d->o);
    ...
    void raydamage(vec &from, vec &to, fpsent *d)
    {
        ...
        else if(d->gunselect!=GUN_FIST && d->gunselect!=GUN_BITE) adddecal(d->gunselect==GUN_RIFLE ? DECAL_CRACK : DECAL_BULLET, to, vec(from).sub(to).normalize(), d->gunselect==GUN_RIFLE ? 8.0f : 2.0f);
    }

    VARP(headshotannotation, 0, 1, 1);
    ...
    void shoot(fpsent *d, const vec &targ)
    {
        ...
        if(d==player1 || d->ai)
        {
            addmsg(N_SHOOT, "rci2i6iv", d, lastmillis-maptime, d->gunselect,
                   ...
                   hits.length(), hits.length()*sizeof(hitmsg)/sizeof(int), hits.getbuf());
            demo::shotfx(d->clientnum, d->gunselect, lastmillis-maptime, from, to);
        }
        ...
        statslog[STATS_SHOTS] += guns[d->gunselect].damage*(d->quadmillis ? 4 : 1)*guns[d->gunselect].rays;

        int gun = d->gunselect;
        if(d && gun >= 0 && gun < MAXWEAPONS)
        {
            d->detaileddamagetotal[gun] += guns[gun].damage*(d->quadmillis ? 4 : 1)*guns[gun].rays;
            if(gun==3 || gun==5) d->lastprojectile = gun;
        }
        if(headshotannotation && d==player1) loopv(players)
        {
            vec buf = players[i]->o;
            buf.add(vec(0, 0, players[i]->eyeheight));
            if(buf.dist(to) < 16) conoutf("\f6headshot! \f4(dist: %.1f)", buf.dist(to));
        }
    }
