...
//            particle_fireball(b.ammopos, 4.8f, PART_EXPLOSION, 0, b.owner[0] ? (strcmp(b.owner, player1->team) ? 0x802020 : 0x2020FF) : 0x208020, 4.8f);
    ...
    {
        if(m_regencapture) return -1;
        return max(0, RESPAWNSECS*1000-(lastmillis-d->lastpain))/1000.0f;
    }
    ...
    void updatebase(int i, const char *owner, const char *enemy, int converted, int ammo)
    {
        ...
        if(owner[0])
        {
            ...
            {
                if(!b.name[0]) conoutf(CON_GAMEINFO, "%s%s captured base %d", getmsgcolorstring(), teamcolor(owner, owner), i+1);
                else if(basenumbers) conoutf(CON_GAMEINFO, "%s%s captured %s (%d)", getmsgcolorstring(), teamcolor(owner, owner), b.name, i+1);
                else conoutf(CON_GAMEINFO, "%s%s captured %s", getmsgcolorstring(), teamcolor(owner, owner), b.name);
                if(!strcmp(owner, player1->team)) playsound(S_V_BASECAP);
            }
        }
        else if(b.owner[0])
        {
            if(!b.name[0]) conoutf(CON_GAMEINFO, "%s%s lost base %d", getmsgcolorstring(), teamcolor(b.owner, b.owner), i+1);
            else if(basenumbers) conoutf(CON_GAMEINFO, "%s%s lost %s (%d)", getmsgcolorstring(), teamcolor(b.owner, b.owner), b.name, i+1);
            else conoutf(CON_GAMEINFO, "%s%s lost %s", getmsgcolorstring(), teamcolor(b.owner, b.owner), b.name);
    ...
    void setscore(int base, const char *team, int total)
    {
        ...
        if(total>=10000)
        {
            event::run(event::COAB, team);
            conoutf(CON_GAMEINFO, "%s%s captured all bases", getmsgcolorstring(), teamcolor(team, team));
        }
