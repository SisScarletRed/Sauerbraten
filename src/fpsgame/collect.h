...
#endif
{
    ...
    void stealtoken(fpsent *d, int id, const vec &o, int team, int yaw, int n, int basenum, int enemyteam, int score)
    {
        ...
        if(bases.inrange(basenum))
        {
            ...
            if(!n)
            {
                event::run(event::SKULL_TAKE, tempformatstring("%d", d->clientnum));
                b.laststeal = lastmillis;
                conoutf(CON_GAMEINFO, "%s%s stole a skull from %s", getmsgcolorstring(), teamcolorname(d), teamcolor("your team", collectbaseteam(enemyteam), "the enemy team"));
    ...
    void deposittokens(fpsent *d, int basenum, int deposited, int team, int score, int flags)
    {
        ...
        d->flags = flags;
        setscore(team, score);
        event::run(event::SKULL_SCORE, tempformatstring("%d", d->clientnum));

        conoutf(CON_GAMEINFO, "%s%s collected %d %s for %s", getmsgcolorstring(), teamcolorname(d), deposited, deposited==1 ? "skull" : "skulls", teamcolor("your team", collectbaseteam(team), "the enemy team"));
        playsound(team==collectteambase(player1->team) ? S_FLAGSCORE : S_FLAGFAIL);

        if(score >= SCORELIMIT)
        {
            conoutf(CON_GAMEINFO, "%s%s collected %d skulls", getmsgcolorstring(), teamcolor("your team", collectbaseteam(team), "the enemy team"), score);
            event::run(event::COAS, tempformatstring("%s", team==1 ? "good" : (team==2 ? "evil" : "unknown")));
        }
    }
