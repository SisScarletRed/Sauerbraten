...
#endif
{
    ...
    struct flag
    {
        ...
        int team, droptime, owntime, runstart;
#ifdef SERVMODE
    ...
    void drawhud(fpsent *d, int w, int h)
    {
        ...
        if(d->state == CS_ALIVE)
        {
            loopv(flags) if(flags[i].owner == d)
            {
                ...
                hud::drawicon(flags[i].team==ctfteamflag(d->team) ? HICON_BLUE_FLAG : HICON_RED_FLAG, HICON_X + 3*HICON_STEP + (d->quadmillis ? HICON_SIZE + HICON_SPACE : 0), HICON_Y, 120);
                if(m_hold)
                {
                    glPushMatrix();
                    glScalef(2, 2, 1);
    ...
    void rendergame()
    {
        loopv(flags)
        {
            ...
            particle_flare(f.spawnloc, vec(f.spawnloc.x, f.spawnloc.y, f.spawnloc.z+512), 1, PART_LFLARE, f.team==ctfteamflag(player1->team) ? 0x0000FF : 0xFF0000, 5);

            vec buf;
            if(raycubelos(vec(f.spawnloc.x, f.spawnloc.y, f.spawnloc.z+512), camera1->o, buf))
                particle_splash(PART_GLOW, 1, 1, vec(f.spawnloc.x, f.spawnloc.y, f.spawnloc.z+512), fs[i]?f.team==ctfteamflag(player1->team) ? 0x0000FF : 0xFF0000:0x000000, 40, 1, 0);

            vec buf2;
            if(raycubelos(vec(f.spawnloc.x, f.spawnloc.y, f.spawnloc.z+10), camera1->o, buf2))
                particle_splash(PART_GLOW, 1, 1, vec(f.spawnloc.x, f.spawnloc.y, f.spawnloc.z+10), fs[i]?f.team==ctfteamflag(player1->team) ? 0x000055 : 0x550000:0x000000, 20, 1, 0);
    ...
    void dropflag(fpsent *d, int i, int version, const vec &droploc)
    {
        ...
        event::run(event::FLAG_DROP, tempformatstring("%d", d->clientnum));
        conoutf(CON_GAMEINFO, "%s%s dropped %s", getmsgcolorstring(), teamcolorname(d), teamcolorflag(f));
    ...
    void flageffect(int i, int team, const vec &from, const vec &to)
    {
        ...
        particle_flare(to, vec(to.x, to.y, to.z+548), 1000, PART_LFLARE, team==ctfteamflag(player1->team) ? 0x7777FF : 0xFF7777, 20);
    }
    ...
    void returnflag(fpsent *d, int i, int version)
    {
        ...
        event::run(event::FLAG_RETURN, tempformatstring("%d", d->clientnum));
        conoutf(CON_GAMEINFO, "%s%s returned %s", getmsgcolorstring(), teamcolorname(d), teamcolorflag(f));
        playsound(S_FLAGRETURN);
        particle_flare(f.spawnloc, vec(f.spawnloc.x, f.spawnloc.y, f.spawnloc.z+548), 1000, PART_LFLARE, f.team==ctfteamflag(player1->team) ? 0x7777FF : 0xFF7777, 20);
        fs[i] = 0;
    }
    ...
    void resetflag(int i, int version, int spawnindex, int team, int score)
    {
        ...
        event::run(event::FLAG_RESET);
        if(shouldeffect)
        {
            conoutf(CON_GAMEINFO, "%s%s reset", getmsgcolorstring(), teamcolorflag(f));
    ...
    void scoreflag(fpsent *d, int relay, int relayversion, int goal, int goalversion, int goalspawn, int team, int score, int dflags)
    {
        ...
        if(flags.inrange(goal))
        {
            ...
            if(m_ctf)
            {
                if(relay >= 0 && flags.inrange(relay))
                {
                    conoutf(CON_GAMEINFO, "%s%s scored for %s (in %.1fs)", getmsgcolorstring(), teamcolorname(d), teamcolor("your team", ctfflagteam(team), "the enemy team"), float(lastmillis-flags[relay].runstart)/1000.0f);
                    if(lastmillis-flags[relay].runstart < 3000)
                        detected_cheat(tempformatstring("flag-hack; scored too fast (%d)", lastmillis-flags[relay].runstart), AC_HIGH, d);
                }
                else
                    conoutf(CON_GAMEINFO, "%s%s scored for %s", getmsgcolorstring(), teamcolorname(d), teamcolor("your team", ctfflagteam(team), "the enemy team"));
            }
            if(m_hold) spawnflag(f);
            if(relay >= 0)
            {
        ...
        else statslog[STATS_FLAGS]++;
        d->flags = dflags;
        event::run(event::FLAG_SCORE, tempformatstring("%d", d->clientnum));
        playsound(team==ctfteamflag(player1->team) ? S_FLAGSCORE : S_FLAGFAIL);
        particle_flare(flags[goal].spawnloc, vec(flags[goal].spawnloc.x, flags[goal].spawnloc.y, flags[goal].spawnloc.z+548), 1000, PART_LFLARE, flags[goal].team==ctfteamflag(player1->team) ? 0x7777FF : 0xFF7777, 20);

        if(score >= FLAGLIMIT)
        {
            if(score==FLAGLIMIT) event::run(event::COAF, tempformatstring("%s", team==1 ? "good" : tempformatstring("%s", team==2 ? "evil" : "unknown")));
            conoutf(CON_GAMEINFO, "%s%s captured %d flags", getmsgcolorstring(), teamcolor("your team", ctfflagteam(team), "the enemy team"), score);
        }
        fs[relay] = 0;
    }
    ...
    void takeflag(fpsent *d, int i, int version)
    {
        ...
        flag &f = flags[i];
        if(f.interploc.dist(d->o) > 4000)
        {
            detected_cheat(tempformatstring("Stole flag from too far away (%d)", (int)f.interploc.dist(d->o)),
                           AC_TESTING
                           , d);
        }
        f.version = version;
        f.interploc = interpflagpos(f, f.interpangle);
        f.interptime = lastmillis;
        event::run(event::FLAG_TAKE, tempformatstring("%d", d->clientnum));
        if(m_hold)
            conoutf(CON_GAMEINFO, "%s%s picked up the flag for %s", getmsgcolorstring(), teamcolorname(d), teamcolor("your team", d->team, "the enemy team"));
        else if(m_protect || f.droptime)
            conoutf(CON_GAMEINFO, "%s%s picked up %s", getmsgcolorstring(), teamcolorname(d), teamcolorflag(f));
        else
        {
            f.runstart = lastmillis;
            conoutf(CON_GAMEINFO, "%s%s stole %s", getmsgcolorstring(), teamcolorname(d), teamcolorflag(f));
        }
        ownflag(i, d, lastmillis);
        playsound(S_FLAGPICKUP);
        particle_flare(f.spawnloc, vec(f.spawnloc.x, f.spawnloc.y, f.spawnloc.z+548), 1000, PART_LFLARE, f.team==ctfteamflag(player1->team) ? 0x0000FF : 0xFF0000, 20);
        fs[i] = 1;
    }
