...
bool _isdemoplayback = false;
extern bool drawcradar;
extern int debugdemo;
ICOMMAND(isdemoplayback, "", (), intret(_isdemoplayback ? 1 : 0));
...
namespace demo
{
    namespace preview
    {
        extern int minz, secz;
        extern bool hastoupdate;
    }
}
...
namespace server
{
    ...
    const char *prettymodename(int n, const char *unknown)
    {
    ...
    void enddemoplayback()
    {
        ...
        _isdemoplayback = false;
        event::run(event::DEMO_END);
    }
    ...
    void setupdemoplayback()
    {
        ...
        else if(demoplayback->read(&hdr, sizeof(demoheader))!=sizeof(demoheader) || (memcmp(hdr.magic, DEMO_MAGIC, sizeof(hdr.magic)) && memcmp(hdr.magic, QUALITY_DEMO_MAGIC, sizeof(hdr.magic))))
        ...
        if(debugquality && debugdemo)
            conoutf("demo: quality-demo: %s, MAGIC: %s, VERSION: %d, PROTOCOL: %d", file, hdr.magic, hdr.version, hdr.protocol);
        msgout:
        if(msg[0])
        {
            DELETEP(demoplayback);
        ...
        if(demo::preview::hastoupdate) execute(tempformatstring("sleep 200 [ demotime %d %d ]", demo::preview::minz, demo::preview::secz));
        _isdemoplayback = true;
        event::run(event::DEMO_START);
    }
