...
QICOMMAND(setcampos, "set the campos to x/y/z (editmode only)", "x,y,z", "fff", (float *x, float *y, float *z),
{
    if(!m_check(game::gamemode, M_EDIT)) return;
    game::player1->o = vec(*x, *y, *z);
    entinmap(game::player1);
    updatedynentcache(game::player1);
})
...
VARP(zoomfov, 10, 35, 90);
QVARP(fov, "change you field of view", 10, 100, 150);
...
void gl_drawframe(int w, int h)
{
    ...
    glClear(GL_DEPTH_BUFFER_BIT|(wireframe && m_check(game::gamemode, M_EDIT) ? GL_COLOR_BUFFER_BIT : 0)|(hasstencil ? GL_STENCIL_BUFFER_BIT : 0));

    glPolygonMode(GL_FRONT_AND_BACK, wireframe && m_check(game::gamemode, M_EDIT) ? GL_LINE : GL_FILL);
    ...
    extern int outline;
    if(outline && m_check(game::gamemode, M_EDIT)) renderoutline();
    ...
    if(wireframe && m_check(game::gamemode, M_EDIT)) glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);
    ...
    if(wireframe && m_check(game::gamemode, M_EDIT)) glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);
...
QVARP(uwscreenfade, "transparency of underwaterscreen", 0, 100, 255);
QVARP(underwaterscreen, "a lil image-overlay when camera1 is in water", 0, 1, 1);
void drawunderwaterscreen(int w, int h)
{
    if(lookupmaterial(camera1->o)!=MAT_WATER || !underwaterscreen || zoom) return;

    defaultshader->set();
    glEnable(GL_TEXTURE_2D);

    static Texture *underwatertex = NULL;
    if(!underwatertex) underwatertex = textureload("data/underwater.png", 3);

    glBlendFunc(GL_ONE, GL_ONE_MINUS_SRC_ALPHA);
    glBindTexture(GL_TEXTURE_2D, underwatertex->id);
    float fade = uwscreenfade/255.0f;
...
bool rmvdscreen = false;
void removedamagescreen() { rmvdscreen = true; }
float fade;
bool atlowhp = false;

void damageblend(int n, bool lowhp)
{
    if(!damagescreen || minimized || (lowhp && fade >= 1.0f)) return;
...
void drawdamagescreen(int w, int h)
{
    ...
    int slowerfade = rmvdscreen ? 1000 : damagescreenfade;
    if(damageblendmillis-lastmillis < damagescreenfade || rmvdscreen)
        fade *= float(damageblendmillis-lastmillis)/slowerfade;
    glColor4f(fade, fade, fade, fade);
    glBegin(GL_TRIANGLE_STRIP);
    glTexCoord2f(0, 0); glVertex2f(0, 0);
    glTexCoord2f(1, 0); glVertex2f(w, 0);
...
VARP(deathcamerascreen, 0, 1, 1);

extern int deathcamerastate;
void drawdeathcamerascreen(int w, int h)
{
    if(deathcamerascreen && deathcamerastate)
    {
        defaultshader->set();
        glEnable(GL_TEXTURE_2D);
        ...
        Texture *t = textureload("packages/hud/cam.png", 3);
...
void crosshairbump()
{
    if(prevsize==crosshairsize || !prevsize) prevsize = crosshairsize;
    if(dofx)
    {
        dofx = false;
        crosshairsize = prevsize;
    }
    dofx = true;
}
...
{
    if(dofx && prevsize==crosshairsize)
    {
        crosshairsize += 20;
        cmillis = lastmillis;
    }
    if(dofx && lastmillis >= (cmillis+10))
    {
        crosshairsize -= 2;
        cmillis = lastmillis;
    }
    if(dofx && crosshairsize <= prevsize)
    {
        crosshairsize = prevsize;
        dofx = false;
        prevsize = 0;
...
void qsetcursor(const char *cur)
{
    ...
    buf.append("data/guicursor");
    if(strlen(cur)) buf.append(cur);
    buf.append(".png");

    copystring(cursor_, buf.str());
}
...
void drawcrosshair(int w, int h)
{
    ...
    {
        Texture *cursor = NULL;
        cursor = textureload(cursor_, 3, true);
        crosshair = cursor;
...
VARP(showfps, 0, 1, 1);
VARP(showfpsgui, 0, 1, 1);
VARP(showfpsscoreboard, 0, 0, 1);
VARP(showfpsmainmenu, 0, 1, 1);
VARP(showfpsediting, 0, 1, 1);
QVARP(showfpsalways, "show timer stats always", 0, 0, 1);
QVARP(showextendedstats, "show more render-time stats to see where spikes are", 0, 1, 1);
VARP(newedithud, 0, 1, 1);
...
extern const char *getsval(char *var),
                  *getcurblendbrushname();
...
void gl_drawhud(int w, int h)
{
    ...
    if(!mainmenu)
    {
        ...
        drawunderwaterscreen(w, h);
        crosshairbumpcheck();
        drawdeathcamerascreen(w, h);
    }
    ...
    if(1) // todo
    {
        int fpsstatlines = 0;
        roffset += FONTH;
        if(((!editmode || showfpsediting) && (!mainmenu || showfpsmainmenu) &&
            (!guiisshowing() || showfpsgui) && (!getvar("scoreboard") || showfpsscoreboard)) ||
           showfpsalways)
        {
            if(showfps)
            {
                // Get fps
                static float prevfps[3] = { 0, 0, 0 }, curfps[3] = { 0, 0, 0 };
                ...
                int tw, th;
                defformatstring(buf1)("FPS: %.3f+%.1f-%.1f", curfps[0], curfps[1], curfps[2]);
                text_bounds(buf1, tw, th);
                fpsstatlines++;
                draw_textf(buf1, conw-tw-10, conh-roffset);
                roffset += FONTH;

                if(showextendedstats)
                {
                    // Get framenanos
                    extern void getframenanos(ullong &avg, ullong &bestdiff, ullong &worstdiff);
                    static ullong prevframenanos[3] = { 0, 0, 0 }, curframenanos[3] = { 0, 0, 0 };
                    memcpy(prevframenanos, curframenanos, sizeof(prevframenanos));
                    ullong nextframenanos[3];
                    getframenanos(nextframenanos[0], nextframenanos[1], nextframenanos[2]);
                    loopi(3) if(prevframenanos[i]==curframenanos[i]) curframenanos[i] = nextframenanos[i];

                    int tw2, th2;
                    defformatstring(buf2)("Framenanos: %d", (int)curframenanos[0]);
                    text_bounds(buf2, tw2, th2);
                    fpsstatlines++;
                    draw_textf(buf2, conw-tw2-10, conh-roffset);
                    roffset += FONTH;

                    // Get lag/secs
                    extern void getlag(float &lag, float &bestdiff, float &worstdiff);
                    static float prevlag[3] = { 0, 0, 0 }, curlag[3] = { 0, 0, 0 };
                    memcpy(prevlag, curlag, sizeof(prevlag));
                    float nextlag[3];
                    getlag(nextlag[0], nextlag[1], nextlag[2]);
                    loopi(3) if(prevlag[i]==curlag[i]) curlag[i] = nextlag[i];

                    int tw3, th3;
                    defformatstring(buf3)("inputchecks/s: %.2fk", curlag[0]/1000.0f);
                    text_bounds(buf3, tw3, th3);
                    fpsstatlines++;
                    draw_textf(buf3, conw-tw3-10, conh-roffset);
                    roffset += FONTH;

                    // Get lagnanos
                    extern void getlagnanos(ullong &avg, ullong &bestdiff, ullong &worstdiff);
                    static ullong prevlagnanos[3] = { 0, 0, 0 }, curlagnanos[3] = { 0, 0, 0 };
                    memcpy(prevlagnanos, curlagnanos, sizeof(prevlagnanos));
                    ullong nextlagnanos[3];
                    getlagnanos(nextlagnanos[0], nextlagnanos[1], nextlagnanos[2]);
                    loopi(3) if(prevlagnanos[i]==curlagnanos[i]) curlagnanos[i] = nextlagnanos[i];

                    int tw4, th4;
                    defformatstring(buf4)("input: %.2f usecs", (float)(curlagnanos[0]/1000.0f));
                    text_bounds(buf4, tw4, th4);
                    fpsstatlines++;
                    draw_textf(buf4, conw-tw4-10, conh-roffset);
                    roffset += FONTH;
                    ...
                    //int tmpw = 0, tmph = showfps ? 5*FONTH : 4*FONTH;
                    //tmpw = tw > tw2 && tw > tw3 && tw > tw4 && tw > tw5 ? tw : tw2 > tw3 && tw2 > tw4 && tw2 > tw5 ? tw2 : tw3 > tw4 && tw3 > tw5 ? tw3 : tw4 > tw5 ? tw4 : tw5;
                    //drawskinrect(conw-tmpw, conh, tmpw, tmph);
                }
            }
        }
        if(editmode && newedithud)
        {
            int twmax = 0,
                lines = 0,
                tw6 = 0;
            ...
            int tw1, th1;
            defformatstring(buf1)("\f%sEntselsnap %s", entselsnap ? "t" : "4", entselsnap ? "ON" : "OFF");
            text_bounds(buf1, tw1, th1);
            twmax = max(twmax, tw1);
            lines++;

            int tw2, th2;
            defformatstring(buf2)("\f%sEntediting %s", entediting ? "t" : "4", entediting ? "ON" : "OFF");
            text_bounds(buf2, tw2, th2);
            twmax = max(twmax, tw2);
            lines++;

            int tw3, th3;
            defformatstring(buf3)("\f%sFullbright %s", fullbright ? "t" : "4", fullbright ? "ON" : "OFF");
            text_bounds(buf3, tw3, th3);
            twmax = max(twmax, tw3);
            lines++;

            int tw4, th4;
            defformatstring(buf4)("\f%sShowmat %s", showmat ? "t" : "4", showmat ? "ON" : "OFF");
            text_bounds(buf4, tw4, th4);
            twmax = max(twmax, tw4);
            lines++;
            ...
            int tw5, th5;
            extern int outline;
            defformatstring(buf5)("\f%sOutline %s", outline ? "t" : "4", outline ? "ON" : "OFF");
            text_bounds(buf5, tw5, th5);
            twmax = max(twmax, tw5);
            lines++;

            if(multiplayer(false) || game::demoplayback)
            {
                int th6;
                formatstring(buf6)("\f%sNompedit %s", nompedit ? "t" : "4", nompedit ? "ON" : "OFF");
                text_bounds(buf6, tw6, th6);
                twmax = max(twmax, tw6);
                lines++;
            }
            int fullheight = lines*FONTH;
            drawskinrect(conw-twmax-10, conh-fpsstatlines*FONTH, twmax+20, fullheight, 0.66f);


            draw_textf(buf1, conw-tw1-10, conh-roffset);
            roffset += FONTH;
            draw_textf(buf2, conw-tw2-10, conh-roffset);
            roffset += FONTH;
            draw_textf(buf3, conw-tw3-10, conh-roffset);
            ...
            draw_textf(buf5, conw-tw5-10, conh-roffset);
            roffset += FONTH;
            if(multiplayer(false) || game::demoplayback) draw_textf(buf6, conw-tw6-10, conh-roffset);
        }
    }
    ...
    if(!hidehud && !mainmenu)
    {
        if(!hidestats)
        {
            ...
            if(editmode || showeditstats)
            {
                ...
                draw_textf("Campos: \ft%s %s %s \f7Cube: \ft%s%d%s", FONTH/2, abovehud, intstr((int)camera1->o.x), intstr((int)camera1->o.y), intstr((int)camera1->o.z), selchildcount<0 ? "1/" : "", abs(selchildcount), showmat && selchildmat > 0 ? getmaterialdesc(selchildmat, ": ") : "");
                abovehud -= FONTH;
                draw_textf("Pos \ft%.2f %.2f %.2f \f7Selsize \ft%d %d %d", FONTH/2, abovehud, getselpos().x, getselpos().y, getselpos().z, sel.s.x, sel.s.y, sel.s.z);
                abovehud -= 2*FONTH;
                draw_textf("wtr: \ft%dk (%d%%) \f7wvt: \ft%dk (%d%%) \fzevt: \ft%dk \f7eva: \ft%dk", FONTH/2, abovehud, wtris/1024, curstats[0], wverts/1024, curstats[1], curstats[2], curstats[3]);
                draw_textf("ond: \ft%d \f7va: \ft%d \f7gl: \ft%d (%d) \f7oq: \ft%d \f7lm: \ft%d \f7rp: \ft%d \f7pvs: \ft%d", FONTH/2, abovehud+FONTH, allocnodes*8, allocva, curstats[4], curstats[5], curstats[6], lightmaps.length(), curstats[7], getnumviewcells());
                draw_textf("\f7Gridsize: \ft%d \f7| Worldsize: \ft%d \f7| Floatspeed: \ft%d", FONTH/2, hmapedit || blendpaintmode ? abovehud-2*FONTH : abovehud-FONTH, gridpower, worldscale, floatspeed);
                if(hmapedit)
                    draw_textf("\f7Heightmap brush \ft%d\f7: \ft%s", FONTH/2, abovehud-FONTH, getvar("hbrushindex"), getsval((char *)"hbrushname"));
                else if(blendpaintmode)
                    draw_textf("\f7Blendpaint mode: \ft%d \f7| Blend brush: \ft%s", FONTH/2, abovehud-FONTH, blendpaintmode, getcurblendbrushname());
                abovehud -= hmapedit || blendpaintmode ? 2*FONTH : FONTH;

                limitgui = abovehud-200;
