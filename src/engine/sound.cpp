...
void installmusicplayer()
{
    execute("playasong = [ music (concatword \"fanatic/\" (+ 1 (rnd 8)) \".ogg\") playasong ]");
}
void setmusicvol(int musicvol);
bool soundsoff = false, musicoff = false;
extern void loadweathersounds();
extern bool weathersoundsloaded;
namespace game
{
    extern bool soundsloaded;
    extern void preloadsounds();
}
VARFP(soundvol, 0, 255, 255,
{
    ...
    {
        if(!game::soundsloaded) game::preloadsounds();
        if(!weathersoundsloaded) loadweathersounds();
    }
});
VARFP(musicvol, 0, 128, 255,
{
    setmusicvol(soundvol ? musicvol : 0);
    if(musicoff && musicvol && soundvol)
    {
        installmusicplayer();
        execute("playasong");
    }
    if(!musicvol || !soundvol) musicoff = true;
});
...
void checkmapsounds()
{
    ...
    {
        if(waterchan==-1) waterchan = playsound(113, NULL, NULL, 0, -1, NULL, NULL, waterchan); // TODO
    }
    else
    {
    ...
    loopv(ents)
    {
        ...
        {
            if(!(e.flags&EF_SOUND) && lookupmaterial(camera1->o)!=MAT_WATER) playsound(e.attr1, NULL, &e, 0, -1);
...
VARP(maxsoundradius, 0, 1000, 10000);
...
VARP(maxsoundsatonce, 0, 100, 100);
...
static bool loadsoundslot(soundslot &slot, bool msg = false)
{
    ...
    conoutf(CON_WARN, "failed to load sample: packages/sounds/%s", slot.sample->name);
    return false;
}
