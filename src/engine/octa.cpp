...
int isemptycube(int tx, int ty, int tz, int tscale)
{
    tx = clamp(tx, 0, worldsize-1);
    ty = clamp(ty, 0, worldsize-1);
    tz = clamp(tz, 0, worldsize-1);
    ...
    if(tscale > scale) return -1;

    while(tscale <= scale)
    {
        scale--;
        size = size/2;
        if(c->children) c = &c->children[octastep(tx, ty, tz, scale)];
        else break;

    }

    if(c->children) return 0;
    else if(c->faces[0] == 0 && c->faces[1] == 0 && c->faces[2] == 0) return 1;
    else
    {
        if(scale+1 > tscale)
        {
            int x = tx-(tx/size)*size;
            int y = ty-(ty/size)*size;
            int z = tz-(tz/size)*size;
