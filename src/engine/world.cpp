...
ICOMMAND(getselface,       "", (),   intret(sel.orient)                                                     );
ICOMMAND(getselgrid,       "", (),   intret(sel.grid)                                                       );
ICOMMAND(getselpos,        "", (), { defformatstring(s)("%d %d %d", sel.o.x, sel.o.y, sel.o.z); result(s); });
ICOMMAND(getselsize,       "", (), { defformatstring(s)("%d %d %d", sel.s.x, sel.s.y, sel.s.z); result(s); });
ICOMMAND(getselcorner,     "", (),   intret(sel.corner)                                                     );
ICOMMAND(getcornerselpos,  "", (), { defformatstring(s)("%d %d", sel.cx, sel.cy); result(s);               });
ICOMMAND(getcornerselsize, "", (), { defformatstring(s)("%d %d", sel.cxs, sel.cys); result(s);             });

ICOMMAND(setselface,       "i",   (int *f),                   sel.orient = *f              );
ICOMMAND(setselgrid,       "i",   (int *g),                   sel.grid = *g                );
ICOMMAND(setselpos,        "iii", (int *x, int *y, int *z),   sel.o = ivec(*x, *y, *z)     );
ICOMMAND(setselsize,       "iii", (int *w, int *h, int *l),   sel.s = ivec(*w, *h, *l)     );
ICOMMAND(setselcorner,     "i",   (int *c),                   sel.corner = *c              );
ICOMMAND(setcornerselpos,  "ii",  (int *x, int *y),         { sel.cx = *x; sel.cy = *y;   });
ICOMMAND(setcornerselsize, "ii",  (int *w, int *h),         { sel.cxs = *w; sel.cys = *h; });
...
void newentity(int type, int a1, int a2, int a3, int a4, int a5, vec pos = vec(0, 0, 0), bool drop = true)
{
    int idx;
    extentity *t = newentity(true, player->o, type, a1, a2, a3, a4, a5, idx);
    if(!t) return;
    if(drop) dropentity(*t);
    else t->o = pos;
    t->type = ET_EMPTY;
    enttoggle(idx);
    makeundoent();
...
COMMAND(entcopy, "");
COMMAND(entpaste, "");
ICOMMAND(entset_pos, "fff", (float *x, float *y, float *z), if(noentedit()) entedit(efocus, e.o = vec(*x, *y, *z)));
ICOMMAND(stackent, "ifff", (int *num, float *spacx, float *spacy, float *spacz),
{
    if(!entgroup.length())
    {
        conoutf("no ents selected");
        return;
    }

    vec lastspace(0, 0, 0);
    int ents = entgroup.length();   //Getting entgroup size BEFORE stacking, creating new ents makes them selected thus they're included into the entgroup
    loopi(ents)
    {
        extentity *ent = entities::getents()[entgroup[i]];
        loopj(*num)
        {
            if(!j) lastspace = ent->o;
            ...
            newentity(ent->type,
                      ent->attr1, ent->attr2, ent->attr3, ent->attr4, ent->attr5,
                      lastspace,
                      false);
        }
    }
...
COMMAND(nearestent, "");
ICOMMAND(entpos, "", (), entfocus(efocus, defformatstring(s)("%f %f %f", e.o.x, e.o.y, e.o.z); result(s)));
