...
ICOMMAND(enumfonts, "s", (char *last),
{
    ...
    {
        if(retnext || !last || last[0]=='\0')
            result(f.name);
        else if(!strcmp(f.name, last))
            retnext = true;
    })
    result("");
})
...
int getcolorbynum(int n)
{
    switch(n)
    {
        case 0: return 0x00FF00; // green: player talk
        case 1: return 0x0000FF; // blue: "echo" command
        case 2: return 0xFFFF00; // yellow: gameplay messages
        case 3: return 0xFF0000; // red: important errors
        case 4: return 0x888888; // gray
        case 5: return 0xFF00FF; // magenta
        case 6: return 0xFF8800; // orange
        case 7: return 0xFFFFFF; // white
        case 8: return 0x000000; // black
        case 9: return 0xBBBBBB; // light gray
    }
    return 0;
}
ICOMMAND(getcolorbynum, "i", (int *c), intret(getcolorbynum(*c)));
...
static void text_color(char c, char *stack, int size, int &sp, bvec color, int a)
{
    ...
    else
    {
        ...
        switch(c)
        {
            case '0': color = bvec(  0, 255,   0); break;   // green: player talk
            case '1': color = bvec(  0,   0, 255); break;   // blue: "echo" command
            case '2': color = bvec(255, 255,   0); break;   // yellow: gameplay messages
            case '3': color = bvec(255,   0,   0); break;   // red: important errors
            case '4': color = bvec(128, 128, 128); break;   // gray
            case '5': color = bvec(255,   0, 255); break;   // magenta
            case '6': color = bvec(255, 128,   0); break;   // orange
            case '7': color = bvec(255, 255, 255); break;   // white
            case '8': color = bvec(0,     0,   0); break;   // black
            case '9': color = bvec(192, 192, 192); break;   // light gray
            case 't': color = bvec( 80, 207, 229); break;   // "Tesseract Blue"
