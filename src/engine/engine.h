...
struct checkcalldepth
{
    ...
    void refresh()
    {
        if((overflow = (++calldepth >= limit)))
        {
            if(!warningshown)
            {
                conoutf("\f3Warning: \f7call recursion limit reached \f3(%s)\f7.", func);
                warningshown = true;
            }
            overflow = true;
        }
    ...
    checkcalldepth(int max, int &cd, bool &cdw, const char *function) :
        calldepth(cd), limit(max), warningshown(cdw), func(function) { refresh(); }
...
#define CHECKCALLDEPTH(max, overflowinst) \
    ...
    checkcalldepth cd(max, __calldepth, __calldepthwarningshown, __FUNCTION__); \
...
public:
    ...
    void start() { running = true; starttime = tick(); }
    void calc() { time = (tick()-starttime)/1000000.0; reset(); }

    benchmark(const char *desc_, bool justinit = false)
    {
        copystring(desc, desc_);
        running = false;
        time = 0.0;
        if(!justinit) start();
    }
    ~benchmark()
    {
        ...
        {
            calc();
            conoutf("%s took: %.6f ms", desc, time);
        }
    }
...
extern void getfps(float &fps, float &bestdiff, float &worstdiff);
...
extern void renderparticles(bool mainpass = false, bool glowonly = false);
