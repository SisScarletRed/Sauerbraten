...
decalrenderer decals[] =
{
    ...
    decalrenderer("<grey>packages/particles/crack.png", DF_ROTATE)
};
...
void renderdecals(bool mainpass)
{
    ...
    loopi(sizeof(decals)/sizeof(decals[0]))
    {
        ...
        if(mainpass)
        {
            if(!decalsneverfade) d.clearfadeddecals();
            d.fadeindecals();
            if(!decalsneverfade) d.fadeoutdecals();
        }
