...
struct serverinfo : pingattempts, extinfo::serverinfodata
{
    ...
    ENetAddress address;
    bool keep;
    const char *password, *countrycode;

    serverinfo() : resolved(UNRESOLVED), keep(false), password(NULL), countrycode(NULL)
    {
...
vector<extinfo::serverinfodata *> getservers()
{
    vector<extinfo::serverinfodata *> result;
    loopv(servers)
    {
        extinfo::serverinfodata *d = static_cast<extinfo::serverinfodata *>(servers[i]);
        result.add(d);
    }
    return result;
}
...
void *getservergameinfo(ENetAddress address)
{
    ...
    {
        serverinfo *s = servers[i];
        if(s->address.host == address.host && s->address.port == address.port) return s->gameinfo;
    }
    return NULL;
}
...
void pingservers()
{
    ...
    loopi(maxservpings ? min(servers.length(), maxservpings) : servers.length())
    {
        ...
        extinfo::requestgameinfo(si.address);
    ...
    if(searchlan)
    {
        ...
        extinfo::requestgameinfo(address);
    }
    lastinfo = totalmillis;
}
...
static bool sicompare(serverinfo *a, serverinfo *b)
{
    ...
    {
        case game::SB_PING:
            if(a->ping > b->ping) return game::sortupsidedown ? true : false;
            if(a->ping < b->ping) return game::sortupsidedown ? false : true;
            break;
        case game::SB_PLAYERS:
            if(a->numplayers < b->numplayers) return game::sortupsidedown ? true : false;
            if(a->numplayers > b->numplayers) return game::sortupsidedown ? false : true;
            break;
        case game::SB_MODE:
            if(a->attr.length()<5) return game::sortupsidedown ? false :true;
            if(b->attr.length()<5) return game::sortupsidedown ? true :false;
            if(a->attr[1] < b->attr[1]) return game::sortupsidedown ? false :true;
            if(a->attr[1] > b->attr[1]) return game::sortupsidedown ? true :false;
            break;
        case game::SB_MAP:
        {
            int cmp = strcmp(a->map, b->map);
            if(cmp != 0) return !game::sortupsidedown ? (cmp<0) : (cmp>0);
            break;
        }
        case game::SB_TIME:
            if(!ac && !bc) break;
            if(a->attr.length()<3) return game::sortupsidedown ? false : true;
            if(a->attr[2] < 0) return game::sortupsidedown ? false : true;
            if(b->attr.length()<3) return game::sortupsidedown ? true : false;
            if(b->attr[2] < 0) return game::sortupsidedown ? true : false;
            if(a->attr[2] < b->attr[2]) return game::sortupsidedown ? true : false;
            if(a->attr[2] > b->attr[2]) return game::sortupsidedown ? false : true;
            break;
        case game::SB_MASTER:
            if(a->attr.length()<5) return game::sortupsidedown ? false :true;
            if(b->attr.length()<5) return game::sortupsidedown ? true :false;
            if(a->attr[4] < b->attr[4]) return game::sortupsidedown ? false :true;
            if(a->attr[4] > b->attr[4]) return game::sortupsidedown ? true :false;
            break;
        case game::SB_HOST:
        {
        ...
        case game::SB_PORT:
            if(a->port < b->port) return game::sortupsidedown ? false : true;
            if(a->port > b->port) return game::sortupsidedown ? true : false;
            break;
        case game::SB_DESCRIPTION:
        {
...
int allplayersonline = 0,
    lastallplayerscheck = 0;
ICOMMAND(getonlineplayers, "", (),
         {
             if(lastallplayerscheck + 1000 < totalmillis)
             {
                 allplayersonline = 0;
                 loopi(servers.length()) allplayersonline += servers[i]->numplayers;
                 lastallplayerscheck = totalmillis;
             }
             intret(allplayersonline);
         });

#include "serverbrowserfavs.h"

VARP(showserverpreviews, 0, 1, 1);
VARP(serverbrowserfavorites, 0, 1, 1);

VAR(filterfull, 0, 0, 1);
VAR(filterempty, 0, 0, 1);
VAR(filterunkown, 0, 0, 1);
VAR(filtercoop, 0, 0, 2);
...
const char *showservers(g3d_gui *cgui, uint *header, int pagemin, int pagemax)
{
    ...
    for(int start = 0; start < servers.length();)
    {
        ...
        loopi(10)
        {
            ...
            for(int j = start; j < end; j++)
            {
                ...
                if(filterunkown && (si.ping < 0 || si.attr.empty() || si.address.host == ENET_HOST_ANY || si.ping == INT_MAX)) continue;
                if(filterdesc[0])
                {
                ...
                if(filtermap[0]) { if(!strstr(si.map, filtermap)) continue; }
                if(!si.attr.empty())
                {
                    if(filtercoop == 1 && si.attr[1]!=1) continue;
                    if(filtercoop == 2 && si.attr[1]==1) continue;
                }
                no++;

                const char *sdesc = si.sdesc;
                if(si.address.host == ENET_HOST_ANY) sdesc = "[unknown host]";
                else if(si.ping == extinfo::serverinfodata::WAITING) sdesc = "[waiting for response]";
                if(game::serverinfoentry(cgui, i, si.name, si.port, sdesc, si.map, sdesc == si.sdesc ? si.ping : -1, si.attr, si.numplayers/*, si.countrycode*/)) sc = &si;
            }
    ...
    {
        extinfo::setserverpreview(sc->name, sc->port);
        cgui->allowautotab(false);
        return newstring("showgui serverextinfo");
    }
...
const char *curmastername = NULL;
int curmasterport = 0;

void retrieveservers(const char *master, vector<char> &data)
{
    string mastername = "";
    int masterport;
    ...
    if(!port)
    {
        conoutf("\f3invald master server: %s", master);
        return;
    }
    ...
    defformatstring(text)("retrieving servers from %s...", mastername);
    renderprogress(0, text);
...
VARP(allowmasterserverscriptsmsg, 0, 1, 1);
VARP(allowmasterserverscripts, 0, 0, 1);
void parsemasterreply(const char *reply, const char *mastername)
{
    ...
    conoutf("received %d server%s from master server%s", servercount, plural(servercount), mastername);
    return;

    invalidcommand:
    if(!allowmasterserverscriptsmsg) return;
    conoutf("\f3master server%s sent an invalid command - executing scripts from master server is disabled for security reasons", mastername);
...
static vector<strtool> masterservers;
...
static void addmasterserver(const char *server, int *port)
{
    ...
    if(!*server)
    {
        conoutf("\f3no master server given");
        intret(0);
        return;
    }
    if(*port > 0xFFFF || *port < 0)
    {
        conoutf("\f3invalid port given");
        intret(0);
        return;
    }
    ...
    if(strstr(server, ":"))
    {
        conoutf("\f3master server should not contain ':'");
        intret(0);
        return;
    }
    if(masterservers.length()+1 >= MAXMASTERSERVERS)
    {
        conoutf("\f3you may only add up to %d master servers", MAXMASTERSERVERS);
        intret(0);
        return;
    }
    strtool master;
    ...
    {
        conoutf("\f3master server (%s) is already added", master.str());
        intret(0);
        return;
    }
...
static void delmasterserver(const char *server, int *port)
{
    ...
    if(index < 0)
    {
        conoutf("\f3master server (%s) is not added", master.str());
        intret(0);
        return;
    }
...
static void listmasterservers()
{
    ...
    conoutf("listing %d master server%s:", count, plural(count));
    {
        extern char *mastername;
        extern int masterport;
    ...
    loopv(masterservers)
    {
        strtool &server = masterservers[i];
        strtool *strs;
        if(server.split(":", &strs) > 1)
        {
            strtool &server = strs[0];
            strtool &port = strs[1];
...
void updatefrommaster()
{
    ...
    loopv(masterservers)
    {
        strtool &master = masterservers[i];
        static strtool mastername;
        mastername.clear();
        data.setsize(0);
