...
namespace game { extern int gamespeed; }

Shader *particleshader = NULL, *particlenotextureshader = NULL;

VARP(particlesize, 20, 100, 500);
QVARP(mapweather, "additional map effects", 0, 1, 1);
...
enum
{
    ...
    PT_EXPAND      = 1<<20, // expands size as it ages
    PT_EXPAND_FAST = 1<<21, // expands size as it ages, faster
    PT_PULSE       = 1<<22  // expands and then shrinks
};
...
struct partrenderer
{
    ...
    virtual particle *addpart(const vec &o, const vec &d, int fade, int color, float size, int gravity = 0, int grow = 0) = 0;
    ...
    void calc(particle *p, int &blend, int &ts, vec &o, vec &d, bool step = true)
    {
        o = p->o;
        d = p->d;
        float speed = game::gamespeed/100.0f;
        ...
        else
        {
            if(p->grow)
            {
                switch(p->grow)
                {
                    case 1: p->size +=  0.01f*speed; break; // 504
                    case 2: p->size +=  0.05f*speed; break; // 501
                    case 3: p->size +=  0.10f*speed; break; // 502
                    case 4: p->size +=  2.00f*speed; break; // 503
                    case 5: p->size += 16.00f*speed; break;
                }
            }
            ...
            if(o.z < p->val && step)
            {
                if(collide >= 0)
                {
                    ...
                    {
                        if(collide && collide < 77)
                            adddecal(collide, vec(o.x, o.y, collidez), vec(p->o).sub(o).normalize(), 2*p->size, p->color, type&PT_RND4 ? (p->flags>>5)&3 : 0);
                        else if(collide && collide>=77 && o.dist(camera1->o)<=32)
                            particle_explodesplash(o, 50, PART_BULLET, 0x3333FF, 0.1f, 250, 4); // rain splash
                    }
                }
                else blend = 0;
            }
...
struct varenderer : partrenderer
{
    ...
    {
        if(!tex) tex = textureload(texname, texclamp);
        if(!strcmp(texname, "packages/particles/glow.png") && parts->size != 5.1f)
...
static partrenderer *parts[] =
{
    ...
    &plasmafireballs,                                                                              // plasma explosion fireball
    ...
    new quadrenderer("packages/particles/muzzleflash1.jpg", PT_PART|PT_FEW|PT_FLIP|PT_TRACK|PT_RND4),  // muzzle flash 1
    new quadrenderer("packages/particles/muzzleflash2.jpg", PT_PART|PT_FEW|PT_FLIP|PT_GLARE|PT_TRACK), // muzzle flash 2
    new quadrenderer("packages/particles/muzzleflash3.jpg", PT_PART|PT_FEW|PT_FLIP|PT_GLARE|PT_TRACK), // muzzle flash 3
    new quadrenderer("packages/particles/muzzleflash4.jpg", PT_PART|PT_FEW|PT_TRACK),                  // muzzle flash 4
    ...
    new quadrenderer("packages/particles/flame1.png", PT_PART|PT_FLIP),                            // flame 1
    new quadrenderer("packages/particles/flame2.png", PT_PART|PT_FLIP),                            // flame 2
    new quadrenderer("packages/particles/flame3.png", PT_PART|PT_FLIP),                            // flame 3
    new quadrenderer("packages/particles/flame4.png", PT_PART|PT_FLIP),                            // flame 4
    new quadrenderer("packages/particles/snow.png", PT_PART|PT_GLARE|PT_FLIP),                     // snow
    new trailrenderer("packages/particles/rain.png", PT_TRAIL|PT_LERP, 77),                        // rain
    new trailrenderer("packages/particles/flare.png", PT_TRAIL),                                   // flare
    new trailrenderer("packages/particles/tele.png", PT_TRAIL),                                    // teleport
    new quadrenderer("packages/particles/glow.png", PT_PART, 78),                                  // "glow" effect
    new taperenderer("packages/particles/lightflare.png", PT_TAPE|PT_GLARE),                       // lightflare
    new quadrenderer("packages/particles/bubble.jpg", PT_PART|PT_GLARE),                           // bubble (underwater
    new quadrenderer("packages/particles/explode.jpg", PT_PART|PT_GLARE),                          // explode
    new taperenderer("packages/particles/smoketrail.png", PT_TAPE|PT_GLARE),                       // smoke-trail

    new quadrenderer("<grey>packages/particles/explosion/flame.png", PT_PART|PT_RND4|PT_GLARE|PT_EXPAND_FAST),   // explosion flame
    new quadrenderer("<grey>packages/particles/explosion/flame.png", PT_PART|PT_RND4|PT_EXPAND|PT_LERP),         // explosion flame
    new quadrenderer("<grey>packages/particles/explosion/flash.png", PT_PART|PT_RND4|PT_GLARE|PT_PULSE),         // explosion flash
    new quadrenderer("<grey>packages/particles/explosion/sparks.png", PT_PART|PT_RND4|PT_GLARE|PT_EXPAND_FAST),  // explosion sparks
    new taperenderer("<grey>packages/particles/explosion/flare.png", PT_TAPE|PT_GLARE|PT_EXPAND),                // explosion flare
    new quadrenderer("<grey>packages/particles/explosion/rsparks.png", PT_PART|PT_RND4|PT_GLARE|PT_EXPAND_FAST), // explosion rsparks
...
void regular_particle_splash(int type, int num, int fade, const vec &p, int color, float size, int radius, int gravity, int delay, bool hover, int grow)
{
    ...
    if(hover) splash(type, color, radius, num, fade, p, size, gravity, false, 2, false, grow);
    else regularsplash(type, color, radius, num, fade, p, size, gravity, delay, false, 0, grow);
}
...
void particle_trail(int type, int fade, const vec &s, const vec &e, int color, float size, int gravity, bool bubbles)
{
    ...
    loopi(steps)
    {
        ...
        if(lookupmaterial(p)==MAT_WATER && (bubbles || type==PART_SMOKE)) newparticle(p, tmp, rnd(250)+250, PART_BUBBLE, 0xFFFFFF, 0.1f, 500);
        else if(!bubbles && lookupmaterial(p)==MAT_WATER && type==PART_SMOKE) continue;
        else if(!bubbles) newparticle(p, tmp, rnd(fade)+fade, type, color, size, gravity);
    }
}
...
void regularshape(int type, int radius, int color, int dir, int num, int fade, const vec &p, float size, int gravity, float vel = 0, vec windoffset = vec(0, 0, 0), bool weather = false, bool weather2 = false, bool b = false)
{
    ...
    loopi(num)
    {
        ...
        if(weather)
        {
            ...
            if(debugquality && debugweather) conoutf("to.z: %f | floorH: %f | dist: %f", to.z, floorHeight, distToFloor);
...
QVARP(mapglow, "an HDR-like effect around enlightened things (like lights)", 0, 1, 1);
VAR(mapglowfireflicker, 1, 34, 1000);
...
static void makeparticles(entity &e)
{
    switch(e.attr1)
    {
        ...
        case 0: //fire and smoke -  <radius> <height> <rgb> - 0 values default to compat for old maps
        {
            ...
            vec occlusioncheck,
                pos(e.o.x, e.o.y, e.o.z+(e.attr3/14));
            if(SDL_GetTicks()-lastglowtimeupdate[0] > (Uint32)mapglowfireflicker)
            {
                radiusplus_[0] = rndscale(5);
                lastglowtimeupdate[0] = SDL_GetTicks();
            }
            if(raycubelos(pos, camera1->o, occlusioncheck) && mapglow)
                particle_flare(pos, pos, 1, PART_GLOW, 0x903020, (radius*10)+radiusplus_[0], NULL);
        ...
        case 4:  //tape - <dir> <length> <rgb>
        {
            ...
            dir=e.attr2;
            dir &= 0x1F;
            if(dir < 15 && dir > 6 && mapweather && e.attr3 >= 32)
                regularshape(PART_RAIN, e.attr3, colorfromattr(!e.attr4 ? e.attr4=0xFFF : e.attr4), 44, 64, 0, e.o, 0.1f, 201, 1000, vec(20, 20, 0), true);
            else if(e.attr2 >= 256)
                regularshape(type, max(1+e.attr3, 1), colorfromattr(e.attr4), e.attr2-256, 5, 200, e.o, size, gravity);
            else
        ...
        case 11: // flame <radius> <height> <rgb> - radius=100, height=100 is the classic size
        {
            ...
            vec occlusioncheck,
                pos(e.o.x, e.o.y, e.o.z+(e.attr3/14));
            if(SDL_GetTicks()-lastglowtimeupdate[1] > (Uint32)mapglowfireflicker)
            {
                radiusplus_[1] = rndscale(5);
                lastglowtimeupdate[1] = SDL_GetTicks();
            }
            if(raycubelos(pos, camera1->o, occlusioncheck) && mapglow)
                particle_flare(pos, pos, 1, PART_GLOW, colorfromattr(e.attr4), (radius*10)+radiusplus_[1], NULL);
            break;
        }
        ...
        {
            if(mapweather)
                regularshape(e.attr1==77 ? PART_SNOW : PART_RAIN, 0, colorfromattr(e.attr4), 44, e.attr5, 0, e.o, e.attr1 == 77 ? 0.25f : 0.1f, e.attr1 == 77 ? 200   : 201, e.attr1 == 77 ? 350   : 1000, vec(e.attr2, e.attr3, 0), true, false, true);
            break;
        }
        case 79: // glow
        {
            vec occlusioncheck;
            if(raycubelos(e.o, camera1->o, occlusioncheck) && mapglow )
...
bool printparticles(extentity &e, char *buf)
{
    switch(e.attr1)
    {
        case 0: case 4: case 7: case 8: case 9: case 10: case 11: case 12: case 13: case 77: case 78:
...
void updateparticles()
{
    ...
    if(!editmode || showparticles)
    {
        ...
        loopv(emitters)
        {
            ...
            if(cullparticles && e.attr1 != 77 && e.attr1 != 78 && pe.maxfade >= 0)
            {
