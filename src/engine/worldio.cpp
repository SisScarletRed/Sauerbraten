...
#ifndef STANDALONE
string ogzname, bakname, cfgname, picname, libname;
...
void setmapfilenames(const char *fname, const char *cname = 0)
{
    ...
    switch(savebak)
    {
        case 3:
        {
            ...
            formatstring(bakname)("packages/%s_%d_%s_%s%d_%s%d.BAK", mapname, timeinfo->tm_mday, months[timeinfo->tm_mon], timeinfo->tm_hour < 10 ? "0" : "" , timeinfo->tm_hour, timeinfo->tm_min < 10 ? "0" : "",  timeinfo->tm_min);
        }
        case 1:
            formatstring(bakname)("packages/%s.BAK", mapname);
            break;
        case 2:
        default:
            formatstring(bakname)("packages/%s_%d.BAK", mapname, totalmillis);
            break;
    }
    formatstring(cfgname)("packages/%s/%s.cfg", pakname, mcfgname);
    formatstring(picname)("packages/%s.jpg", mapname);
	formatstring(libname)("packages/%s.lgz", mapname);
...
QVARP(writeprotectogz, "don't save to protected mapnames", 0, 0, 1);

extern bool save_library(string &libname);
bool save_world(const char *mname, bool nolms)
{
    if(!*mname) mname = game::getclientmap();
    if(writeprotectogz && fileexists(mname, "w"))
    {
        conoutf(CON_ERROR, "save_world: \f3#error: \f7could not write \f4%s.ogz\f7; file exists", mname);
        return false;
    }
...
VAR(weatherfiledbg, 0, 0, 1);
void writeweatherfile()
{
	defformatstring(file)("data/ents/%s.se", game::getclientmap());
	stream *f = openfile(file, "w");
	if(!f)
	{
		conoutf("weather file does not exist/could not be created");
		return;
	}
	...
	{
		entity &tmp = *entities::getents()[i];
		if((tmp.type == ET_SOUND && tmp.attr5 >= 1) || (tmp.type == ET_PARTICLES && tmp.attr1 >= 77)  || tmp.type == 31)
		{
			f->printf("ENT_%i %i %i %i %i %i %i %f %f %f\n", numents+1, tmp.type, tmp.attr1, tmp.attr2, tmp.attr3, tmp.attr4, tmp.attr5, tmp.o.x, tmp.o.y, tmp.o.z);
			if(weatherfiledbg) conoutf("NUM: %i | TYPE: %s | ATTRS: %i %i %i %i %i | POS: %f %f %f", numents+1, tmp.type==5 ? "particles" : "sound", tmp.attr1, tmp.attr2, tmp.attr3, tmp.attr4, tmp.attr5, tmp.o.x, tmp.o.y, tmp.o.z);
			numents++;
		}
	}

	conoutf("Wrote %i entities to %s", numents, file);

	delete f;
}

VARP(loadweather, 0, 1, 1);

bool weathersoundsloaded = false;
void loadweathersounds()
{
	execfile("data/weather/ambience.cfg");
	execfile("packages/sounds/ambience/package.cfg");
	execfile("packages/sounds/fanatic/package.cfg");
	execfile("packages/sounds/kaiser/fx/package.cfg");
	execfile("data/weather/ambience2.cfg");
	execfile("data/weather/ambience3_doomish.cfg");
	execfile("data/weather/ambience4.cfg");
	weathersoundsloaded = true;
}
...
void readweatherfile()
{
	...
	weathersoundsloaded = false;
	renderprogress(0, "attempting to read weather file");
	if(soundvol) loadweathersounds();
	defformatstring(file)("data/ents/%s.se", game::getclientmap());
	stream *f = openfile(file, "r");
	char buf[512];
	int entnum, numents = 0;
	if(!f)
	{
        conoutf("weather file does not exist - no additional map effects.");
        return;
    }
    renderprogress(0, "reading weather file");

	conoutf("Found weather file for %s. Loaded additional map effects.", game::getclientmap());
	vector<extentity *> &ents = entities::getents();

        while(f->getline(buf, sizeof(buf)))
        {
            int type, a1, a2, a3, a4, a5;
            float x, y, z;
            sscanf(buf, "ENT_%i %i %i %i %i %i %i %f %f %f", &entnum, &type, &a1, &a2, &a3, &a4, &a5, &x, &y, &z);
            if(weatherfiledbg) conoutf("NUM: %i | TYPE: %s | ATTRS: %i %i %i %i %i | POS: %f %f %f", entnum, type==5 ? "particles" : "sound", a1, a2, a3, a4, a5, x, y, z);
	...
	conoutf("Read %i entities from %s", numents, file);
...
vector<mapnametitle> mntitles;

const char *getmaptitle(const char *mname)
{
	loopv(mntitles)
		if(!strcmp(mntitles[i].mname, mname)) return mntitles[i].maptitle;

	string pakname, mapname, mcfgname;
    getmapfilenames(mname, NULL, pakname, mapname, mcfgname);
	defformatstring(filename)("packages/%s.ogz", mapname);
	path(filename);
	stream *d = opengzfile(filename, "rb");
	if(!d) return NULL;

	octaheader hdr;
    if(d->read(&hdr, 7*sizeof(int))!=int(7*sizeof(int))) { delete d; return NULL; }
    lilswap(&hdr.version, 6);
    if(memcmp(hdr.magic, "OCTA", 4) || hdr.worldsize <= 0|| hdr.numents < 0) { delete d; return NULL; }
    if(hdr.version>MAPVERSION) { delete d; return NULL; }

    if(hdr.version <= 28)
    {
		compatheader chdr;
        if(d->read(&chdr.lightprecision, sizeof(chdr) - 7*sizeof(int)) != int(sizeof(chdr) - 7*sizeof(int))) { delete d; return NULL; }
    	mapnametitle &mpt = mntitles.add();
		mpt.mname = newstring(mname);
		mpt.maptitle = newstring(chdr.maptitle);
		delete d;
		return mpt.maptitle;
	}
    else
    {
        int extra = 0;
        if(hdr.version <= 29) extra++;
        if(d->read(&hdr.blendmap, sizeof(hdr) - (7+extra)*sizeof(int)) != (size_t)(sizeof(hdr) - (7+extra)*sizeof(int))) { delete d; return NULL; }
		loopi(hdr.numvars)
		{
			int type = d->getchar(), ilen = d->getlil<ushort>();
			string name;
			d->read(name, min(ilen, MAXSTRLEN-1));
			name[min(ilen, MAXSTRLEN-1)] = '\0';
			if(ilen >= MAXSTRLEN) d->seek(ilen - (MAXSTRLEN-1), SEEK_CUR);

			switch(type)
			{
				case ID_VAR: d->getlil<int>(); break;
				case ID_FVAR: d->getlil<float>(); break;
				case ID_SVAR:
				{
					int slen = d->getlil<ushort>();
					string val;
					d->read(val, min(slen, MAXSTRLEN-1));
					val[min(slen, MAXSTRLEN-1)] = '\0';
					if(slen >= MAXSTRLEN) d->seek(slen - (MAXSTRLEN-1), SEEK_CUR);
					if(!strcmp(name, "maptitle"))
                    {
						mapnametitle &mpt = mntitles.add();
						mpt.mname = newstring(mname);
						mpt.maptitle = newstring(val);
						delete d;
						return mpt.maptitle;
					}
					break;
...
ICOMMAND(getmaptitle, "s", (const char *mapn), result(getmaptitle(mapn)));

// TODO: finish this properly
void checkmaploadcanceled()
{
...
extern bool load_library(string &libname);
bool showloadingscoreboard = true;

bool load_world(const char *mname, const char *cname)        // still supports all map formats that have existed since the earliest cube betas!
{
    if(connecttoserver) showloadingscoreboard = true;
    else showloadingscoreboard = false;
...
COMMAND(writeweatherfile, "");
COMMAND(readweatherfile, "");
