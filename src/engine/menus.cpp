...
extern int show_hand, show_text;

bool updatetcurrent = false;
int newtcurrent = 0;
QICOMMAND(gototab, "open this tab", "tab", "i", (int *tab),
{
    updatetcurrent = true;
    newtcurrent = *tab;
})

#define GUI_TITLE_COLOR  0xDDDDDD
#define GUI_BUTTON_COLOR 0xFFFFFF
#define GUI_TEXT_COLOR   0xAAFFAA
...
bool thisguiisshowing(const char *name)
{
    menu *m = guis.access(name);
    if(m && guistack.find(m) >= 0) return true;
    return false;
}
ICOMMAND(isguishowing, "s", (const char *name), intret(thisguiisshowing(name) ? 1 : 0));
...
extern int hoverstart;
void popgui()
{
    menu *m = guistack.pop();
    m->clear();
    if(m->name) event::run(event::CLOSE_GUI, m->name);
    hoverstart = 0;
}
...
void pushgui(menu *m, int pos = -1)
{
    ...
    if(m->name) event::run(event::SHOW_GUI, m->name);
    hoverstart = 0;
}
...
extern void startboxspin(bool left);
...
void togglegui(const char *name)
{
    menu *m = guis.access(name);
    if(!m) return;
    int pos = guistack.find(m);
    if(pos<0) { startboxspin(false); pushgui(m); }
    else { startboxspin(true); removegui(m); }
}
...
int cleargui(int n)
{
    ...
    playsound(S_GUI_OPEN);
    if(!guistack.empty()) startboxspin(true);
    return clear;
}
...
void guibutton(char *name, char *action, char *icon, char *desc)
{
    if(!cgui) return;
    bool hideicon = !strcmp(icon, "0");
    int ret = cgui->button(name, GUI_BUTTON_COLOR, hideicon ? NULL : (icon[0] ? icon : (strstr(action, "showgui") ? "menu" : "action")), desc[0] ? desc : NULL);
    if(ret&G3D_UP)
    {
...
void guitext(char *name, char *icon, char *desc)
{
    bool hideicon = !strcmp(icon, "0");
    if(cgui) cgui->text(name, !hideicon && icon[0] ? GUI_BUTTON_COLOR : GUI_TEXT_COLOR, hideicon ? NULL : (icon[0] ? icon : "info"), desc[0] ? desc : NULL);
}

void guititle(char *name, char *desc)
{
    if(cgui) cgui->title(name, GUI_TITLE_COLOR, desc[0] ? desc : NULL);
}

void guititleleftalign(char *name, char *desc)
{
    if(cgui) cgui->text(name, GUI_TITLE_COLOR, NULL, desc[0] ? desc : NULL);
}
...
void guicheckbox(char *name, char *var, float *on, float *off, char *onchange, char *desc)
{
    bool enabled = getfval(var)!=*off;
    int ret = cgui->button(name, GUI_BUTTON_COLOR, enabled ? "checkbox_on" : "checkbox_off", desc[0] ? desc : NULL);
    if(cgui && ret&G3D_UP)
    {
...
void guiradio(char *name, char *var, float *n, char *onchange, char *desc)
{
    bool enabled = getfval(var)==*n;
    int ret = cgui->button(name, GUI_BUTTON_COLOR, enabled ? "radio_on" : "radio_off", desc[0] ? desc : NULL);
    if(cgui && ret&G3D_UP)
    {
...
void guibitfield(char *name, char *var, int *mask, char *onchange, char *desc)
{
    int val = getval(var);
    bool enabled = (val & *mask) != 0;
    int ret = cgui->button(name, GUI_BUTTON_COLOR, enabled ? "checkbox_on" : "checkbox_off", desc[0] ? desc : NULL);
    if(cgui && ret&G3D_UP) updateval(var, enabled ? val & ~*mask : val | *mask, onchange);
    else if(ret&G3D_ROLLOVER)
        qsetcursor("_hand");
}
...
{
    menu *m = guistack[guistack.length()-1];
    if(!m) return;
    if(m->header) DELETEA(m->header);
    if(header) m->header = header && header[0] ? newstring(header) : NULL;
}
...
namespace extinfo
{
    extern void showplayersgui(g3d_gui *g);
    extern void showserverpreview(g3d_gui *g);
}

void guiplayers()
{
    if(cgui) extinfo::showplayersgui(cgui);
}

void guiserverextinfo()
{
    if(cgui) extinfo::showserverpreview(cgui);
}
...
void guitextscale(float *scale, uint *contents)
{
    float oldscale = curfont->scale;
    curfont->scale = *scale;
    execute(contents);
    curfont->scale = oldscale;
}

void guibackground(int *color, int *w, int *h)
{
    if(!cgui) return;
    cgui->background(*color, *w, *h);
}
...
COMMAND(guitextscale, "fe");
COMMAND(guibackground, "iii");
COMMAND(guimerge, "e");

COMMAND(guilist, "e");
COMMAND(guialign, "ie");
COMMAND(guititle, "ss");
COMMAND(guititleleftalign, "ss");
COMMAND(guibar, "");
COMMAND(guistrut, "fi");
COMMAND(guispring, "i");
COMMAND(guicolumn, "i");
...
COMMAND(guiradio, "ssfss");
COMMAND(guibitfield, "ssiss");
COMMAND(guicheckbox, "ssffss");
COMMAND(guitab, "s");
...
ICOMMAND(guifont, "se", (const char *font, uint *body),
{
    pushfont();
    setfont(font);
    execute(body);
...
namespace game { extern char *showserverdemos(g3d_gui *g); }
namespace demo
{
    namespace preview
    {
...
void guiserverdemos()
{
	if(cgui)
	{
		char *command = game::showserverdemos(cgui);
		if(command)
		{
			updatelater.add().schedule(command);
			if(shouldclearmenu) clearlater = true;
...
COMMAND(guiserverdemos, "s");

// demo-preview
void guidemoextevents(uint *bottom)
{
	if(cgui)
	{
		char *command = demo::preview::showdemohappenings(cgui, bottom);
		if(command)
		{
			updatelater.add().schedule(command);
			if(shouldclearmenu) clearlater = true;
...
COMMAND(guidemoextevents, "e");

void guidemoextinfo()
{
	if(cgui)
	{
		char *command = demo::preview::showdemoinfo(cgui);
		if(command)
		{
			updatelater.add().schedule(command);
			if(shouldclearmenu) clearlater = true;
...
COMMAND(guidemoextinfo, "s");

void guidemobrowser()
{
	if(cgui)
	{
		char *command = demo::preview::demobrowser(cgui);
		if(command)
		{
			updatelater.add().schedule(command);
			if(shouldclearmenu) clearlater = true;
...
COMMAND(guidemobrowser, "s");

namespace master { extern void rendermasterboard(g3d_gui *g); }
void guimaster()
{
    if(cgui) master::rendermasterboard(cgui);
}
COMMAND(guimaster, "");
