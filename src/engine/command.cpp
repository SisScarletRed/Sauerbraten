...
QICOMMAND(abort, "calls \"abort()\"", "", "", (), abort());
QICOMMAND(system, "calls a system command *cmd*", "cmd", "s", (char *cmd),
{
    char *command = cmd;
    int ret = system(command);
    if(ret==-1) conoutf("\f3system: error -1");
    else if(ret > 0) conoutf("\f3system: exit: %d", ret);
    else conoutf("system: %s executed", cmd);
});
...
QVARP(debugquality, "debug messages for QuEd", 0, 0, 1);
...
int variable(const char *name, const char *description, int min, int cur, int max, int *storage, identfun fun, int flags)
{
    addident(ident(ID_VAR, name, description, "", min, max, storage, (void *)fun, flags));
    return cur;
}
...
float fvariable(const char *name, const char *description, float min, float cur, float max, float *storage, identfun fun, int flags)
{
    addident(ident(ID_FVAR, name, description, "", min, max, storage, (void *)fun, flags));
    return cur;
}
...
char *svariable(const char *name, const char *description, const char *cur, char **storage, identfun fun, int flags)
{
    addident(ident(ID_SVAR, name, description, "", storage, (void *)fun, flags));
    return newstring(cur);
}
...
QICOMMAND(getalias, "returns the ident-alias of *name", "name", "s", (char *s), result(getalias(s)));

void writeinfile(char *file, char *stuff, int *append = 0)
{
    stream *f = openfile(file, *append ? "a" : "w");
    if(!f) { conoutf("error opening/creating %s", file); return; }

    f->printf(stuff);
    f->close();
}

QCOMMAND(writeinfile, "write in a file, with the option to append or overwrite", "file,stuff,append", "ssi");
...
bool addcommand(const char *name, const char *description, const char *params, identfun fun, const char *args)
{
    uint argmask = 0;
    int numargs = 0;
    bool limit = true;
    ...
    if(debugquality) conoutf("addcommand: name: %s, desc: %s, params: %s, args: %s", name, description, params, args);
    if(limit && numargs > MAXCOMARGS) fatal("builtin %s declared with too many args: %d", name, numargs);
    addident(ident(ID_COMMAND, name, description, params, args, argmask, numargs, (void *)fun));
    return false;
}
...
static bool compileword(vector<uint> &code, const char *&p, int wordtype, char *&word, int &wordlen)
{
    CHECKCALLDEPTH(250, return false);
    skipcomments(p);
    switch(*p)
    {
...
QICOMMAND(ifnotdeclared, "execute *code* if *name* is not declared", "name,code", "ss", (const char *name, const char *code), if(!getcsident(name)) execute(code));
...
const char *currentcfgfile = NULL;
bool execfile(const char *cfgfile, bool msg)
{
    ...
    if(debugquality) conoutf("file %s executed", cfgfile);
    return true;
}
...
QICOMMAND(isnumeric, "returns 1 if *string* is numeric and 0 if not", "string", "s", (const char *s), intret(isnumeric(s) ? 1 : 0));
...
QICOMMAND(getextension, "returns the assumed extension of the file", "file", "s", (char *str),
...
void cutextension(char *str, char *ext)
{
    ...
    loopirev(len)
    {
        if(str[i]=='.') { pos = &str[i]; break; }
        else if(str[i]=='/' || str[i]=='\\') break;
    }
	if(ext && ext[0]=='.') ext++;
	if(pos && pos+1 && (!ext || !strcmp(pos+1, ext))) pos[0] = '\0';
}

QICOMMAND(cutextension, "returns the file without the given extension or any assumed, if ext is not given", "file,ext", "ss", (char *str, char *ext),
...
QICOMMAND(gettimestr, "returns the time with the given format", "format,forcecase", "si", (const char *format, int *forcecase), gettimestr(format, *forcecase!=0));
...
void writecfg(const char *name)
{
    ...
    {
        ident &id = *ids[i];
        if(id.flags&IDF_PERSIST || id.flags&IDF_QUALITY) switch(id.type)
        {
...
static int isloop = 0;
QICOMMAND(break, "breaks the current loop instantly", "", "", (), if(isloop) breakinst = true);
...
QICOMMAND(while, "the normal while command you know from other languages", "condition,body", "ee", (uint *cond, uint *body), isloop++; while(executebool(cond) && !breakinst) execute(body); isloop--; breakinst = false);
...
static void roundf_(float *a, int *b)
{
    ...
    strtool fmt(fmtbuf, sizeof(fmtbuf));
    fmt = "%.0";
#ifdef WIN32
...
QCOMMANDN(roundf, "returns *float* rounded to *int* decimals", "float,int", roundf_, "fi");
...
ICOMMAND(toupper, "s", (char *s),
{
    char *commandret = newstring(s);
    for(;*commandret;commandret++) *commandret = toupper(*commandret);
    commandret -= strlen(s);
    stringret(commandret));
}
ICOMMAND(tolower, "s", (char *s),
{
    char *commandret = newstring(s);
    for(;*commandret;commandret++) *commandret = tolower(*commandret);
    commandret -= strlen(s);
    stringret(commandret));
}
...
namespace scriptvector
{
    ...
    QCOMMAND(scriptvector, "declare a new scriptvector", "name,index,value,nowarning", "sssi");
    ...
    QCOMMAND(delscriptvectorindex, "finds & deletes *index* in *name*-scriptvector", "name,index", "ss");
    ...
    QCOMMAND(delscriptvector, "delete scriptvector *name*", "name", "s");
    ...
    QCOMMAND(loopscriptvector, "loop through the scriptvector *name*", "name,index,value,body,start,end", "srreii");
    ...
    QCOMMAND(scriptvectorlength, "return the length of scriptvector *name*", "name", "s");
    ...
    QCOMMAND(writescriptvector, "save scriptvector *name* to *file*", "name,file", "ss");
    ...
    QCOMMAND(loadscriptvector, "load a scriptvector from *file*", "file", "s");
}
