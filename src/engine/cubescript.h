...
void getserverdescription()
{
    ...
    if (curpeer && enet_address_get_host_ip(&curpeer->address, hostname, sizeof(hostname)) >= 0)
    {
        if(*servinfo) res = servinfo;
        else if(curpeer)
        {
...
void formattimestamp(int *timestamp, const char *fmt)
{
    ...
    if (ts < 0)
    {
        conoutf("\f3invalid timestamp passed to formattimestamp");
        return;
    }
    ...
    for(const char *s = fmt; *s; s++)
    {
        ...
        switch (*++s)
        {
            ...
            default:
                conoutf("\f3invalid format specifier '%c' passed to formattimestamp", *s);
                return;
        }
    }
...
ICOMMAND(getosstring, "", (), result(getosstring()));
