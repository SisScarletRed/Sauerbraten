...
float slopez = 0.5f;
float stairheight = 4.1f;
float velocity = 1.0f;
float wallz = 0.2f;
int jesus = 0;

ICOMMAND(floorz, "f", (float *f),
{
    if(!m_check(game::gamemode, M_EDIT))
    {
        conoutf(CON_ERROR, "floorz: \f3#error: \f7operation only allowed in edit mode");
        return;
    }
    else if(*f) floorz = *f;
});

ICOMMAND(gravity, "f", (float *f),
{
    if(!m_check(game::gamemode, M_EDIT))
    {
        conoutf(CON_ERROR, "gravity: \f3#error: \f7operation only allowed in edit mode");
        return;
    }
    else if(*f) gravity = *f;
});

ICOMMAND(jumpvel, "f", (float *f),
{
    if(!m_check(game::gamemode, M_EDIT))
    {
        conoutf(CON_ERROR, "jumpvel: \f3#error: \f7operation only allowed in edit mode");
        return;
    }
    else if(*f) jumpvel = *f;
});

ICOMMAND(slopez, "f", (float *f),
{
    if(!m_check(game::gamemode, M_EDIT))
    {
        conoutf(CON_ERROR, "slopez: \f3#error: \f7operation only allowed in edit mode");
        return;
    }
    else if(*f) slopez = *f;
});

ICOMMAND(stairheight, "f", (float *f),
{
    if(!m_check(game::gamemode, M_EDIT))
    {
        conoutf(CON_ERROR, "stairheight: \f3#error: \f7operation only allowed in edit mode");
        return;
    }
    else if(*f) stairheight = *f;
});

ICOMMAND(velocity, "f", (float *f),
{
    if(!m_check(game::gamemode, M_EDIT))
    {
        conoutf(CON_ERROR, "velocity: \f3#error: \f7operation only allowed in edit mode");
        return;
    }
    else velocity = *f;
});

ICOMMAND(wallz, "f", (float *f),
{
    if(!m_check(game::gamemode, M_EDIT))
    {
        conoutf(CON_ERROR, "wallz: \f3#error: \f7operation only allowed in edit mode");
        return;
    }
    else if(*f) wallz = *f;
});

ICOMMAND(jesus, "i", (int *i),
{
    if(!m_check(game::gamemode, M_EDIT))
    {
        conoutf(CON_ERROR, "jesus: \f3#error: \f7operation only allowed in edit mode");
        return;
    }
    else jesus = *i;
});
...
VARP(collisions, 0, 1, 1);

bool plcollide(physent *d, const vec &dir)    // collide with player or monster
{
    if(d->type==ENT_CAMERA || d->state!=CS_ALIVE || (m_check(game::gamemode, M_EDIT) && !collisions)) return false;
    loopdynentcache(x, y, d->o, d->radius)
    {
        const vector<physent *> &dynents = checkdynentcache(x, y);
        loopv(dynents)
        {
            ...
            switch(d->collidetype)
            {
                ...
                case COLLIDE_ELLIPSE_PRECISE:
                    if(o->collidetype == COLLIDE_OBB)
                    {
                        if(!ellipseboxcollide(d, dir, o->o, vec(0, 0, 0), o->yaw, o->xradius, o->yradius, o->aboveeye, o->eyeheight)) continue;
                    }
                    else if(!ellipsecollide(d, dir, o->o, vec(0, 0, 0), o->yaw, o->xradius, o->yradius, o->aboveeye, o->eyeheight)) continue;
                    break;
                case COLLIDE_OBB:
                    if(o->collidetype == COLLIDE_OBB)
                    {
                        if(!plcollide<mpr::EntOBB, mpr::EntOBB>(d, dir, o)) continue;
                    }
                    else if(!plcollide<mpr::EntOBB, mpr::EntCylinder>(d, dir, o)) continue;
                    break;
                default: continue;
            }
            collideplayer = o;
...
static inline bool octacollide(physent *d, const vec &dir, float cutoff, const ivec &bo, const ivec &bs, const cube *c, const ivec &cor, int size) // collide with octants
{
    loopoctabox(cor, size, bo, bs)
    {
        ...
        {
            bool solid = false;
            if((c[i].material&MATF_VOLUME)==MAT_WATER) solid = jesus ? true : false;
...
VARP(walljump, 0, 0, 1);
FVARP(walljumpvel, 0, 1.0f, 10.0f);
...
void slideagainst(physent *d, vec &dir, const vec &obstacle, bool foundfloor, bool slidecollide)
{
    ...
    if(d->jumping && walljump && m_check(game::gamemode, M_EDIT))
    {
        vec velold(d->vel);
        ...
        const float velcur  = d->vel.magnitude2();
        const float velwall = d->vel.dot(wall);
        const float veladd  = d->maxspeed - velwall;

        d->vel.add(vec(wall).mul(veladd));
        d->vel.z = jumpvel * max(velcur / d->maxspeed, walljumpvel);

        recalcdir(d, velold, dir);
        d->jumping = false;
...
FVARP(maxroll, 0, 0, 20);
FVAR(straferoll, 0, 0.033f, 90);
FVAR(faderoll, 0, 0.95f, 1);
VAR(floatspeed, 1, 100, INT_MAX);
VARFP(airjump, 0, 0, 1, airjump ? execute("bunnyhop 0") : NULL);
VARFP(bunnyhop, 0, 0, 1, bunnyhop ? execute("airjump 0") : NULL);
...
void modifyvelocity(physent *pl, bool local, bool water, bool floating, int curtime)
{
    ...
    else if(pl->physstate >= PHYS_SLOPE || water)
    {
        ...
        if(pl->jumping)
        {
            if(m_check(game::gamemode, M_EDIT) && bunnyhop) pl->jumping = true;
            else pl->jumping = false;
            if(m_check(game::gamemode, M_EDIT) && airjump)
            {
...
bool moveplayer(physent *pl, int moveres, bool local, int curtime)
{
    ...
    else                        // apply velocity with collision
    {
        ...
        if(timeinair > 1000 && !pl->timeinair && !water) // if we land after long time must have been a high jump, make thud sound
            game::physicstrigger(pl, local, -1, 0);
    }
...
ICOMMAND(jump,   "D", (int *down), { if(!*down || game::canjump()) player->jumping = *down!=0; if(*down) game::setautorespawn(); });
ICOMMAND(attack, "D", (int *down), { game::doattack(*down!=0); });

#ifndef MAX_MOUSE_DELTA
#define MAX_MOUSE_DELTA INT_MAX
#endif
...
ICOMMAND(lookup,    "i", (int* speed), { sendmotionevent(0, +(*speed)); });
ICOMMAND(lookdown,  "i", (int* speed), { sendmotionevent(0, -(*speed)); });
ICOMMAND(lookleft,  "i", (int* speed), { sendmotionevent(-(*speed), 0); });
ICOMMAND(lookright, "i", (int* speed), { sendmotionevent(+(*speed), 0); });
