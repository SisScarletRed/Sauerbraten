...
// In-game player-model-skin switch

// TODO: setup skins, menu and fix code!!
...
enum skinnames
{
	...
	SKIN_MRFIXIT_NEUTRAL,
	SKIN_IRON_NEUTRAL,
	SKIN_OGRO_NEUTRAL,
	SKIN_INKY_NEUTRAL,
	SKIN_CC_NEUTRAL,

	SKIN_MRFIXIT_RED_HANDS,
	SKIN_MRFIXIT_BLUE_HANDS,
	SKIN_MRFIXIT_NEUTRAL_HANDS,

	SKIN_IRON_RED_HANDS,
	SKIN_IRON_BLUE_HANDS,
	SKIN_IRON_NEUTRAL_HANDS,
...
#define SKINMARKER "_q_" // every modded skin is included this way: name is <usualskin>_q_<name> e.g. flag_q_bright.jpg

struct skininfo
{
    const char *folder, *ext, *part1, *part2, *cfg;
    vector<char *> versions;
} skininfos[SKIN_MAX] = // TODO: more model skins & dds->png
{
	{ "mrfixit",		   "dds", "Bodyred",		  "Headred",		 "red/md5.cfg" },
	{ "snoutx10k",		   "png", "lower_r",	      "upper_r",		 "red/md5.cfg" },
	{ "ogro2",			   "jpg", "red",			  NULL,				 "red/iqm.cfg" },
	{ "inky",			   "dds", "inky_red",		  "inky_wings_red",	 "red/md5.cfg" },
	{ "captaincannon",	   "dds", "cc_body_red",      "cc_head_red",	 "red/md5.cfg" },

	{ "mrfixit",		   "dds", "Bodyblue",		  "Headblue",		 "blue/md5.cfg" },
	{ "snoutx10k",	   	   "png", "lower_b",		  "upper_b",		 "blue/md5.cfg" },
	{ "ogro2",			   "jpg", "blue",			  NULL,				 "blue/iqm.cfg" },
	{ "inky",			   "dds", "inky_blue",		  "inky_wings_blue", "blue/md5.cfg" },
	{ "captaincannon",	   "dds", "cc_body_blue",	  "cc_head_blue",	 "blue/md5.cfg" },

	{ "mrfixit",		   "dds", "Body",			  "Head",			 "md5.cfg" },
	{ "snoutx10k",		   "dds", "lower",			  "upper",			 "md5.cfg" },
	{ "ogro2",			   "jpg", "green",			  NULL,				 "iqm.cfg" },
	{ "inky",			   "dds", "inky",			  "inky_wings",		 "md5.cfg" },
	{ "captaincannon",	   "dds", "cc_body",		  "cc_head",		 "md5.cfg" },

	{ "mrfixit/hudguns",   "dds", "fixit_hands_red",  NULL,			     "md5.cfg" },
	{ "mrfixit/hudguns",   "dds", "fixit_hands_blue", NULL,			     "md5.cfg" },
	{ "mrfixit/hudguns",   "dds", "fixit_hands",      NULL,				 "md5.cfg" },

	{ "snoutx10k/hudguns", "dds", "snout_hands_red",  NULL,			     "md5.cfg" },
	{ "snoutx10k/hudguns", "dds", "snout_hands_blue", NULL,		         "md5.cfg" },
	{ "snoutx10k/hudguns", "dds", "snout_hands",      NULL,				 "md5.cfg" },

	{ "flags/red",		   "jpg", "fskin",			  NULL,				 "md2.cfg" },
	{ "flags/blue",		   "jpg", "fskin",			  NULL,				 "md2.cfg" },
	{ "flags/neutral",	   "jpg", "fskin",			  NULL,				 "md2.cfg" }
};

void rewritemodelcfg(int skin, int version)
{
	skininfo &a = skininfos[skin];
	defformatstring(cfg)("packages/models/%s/%s", a.folder, a.cfg);
	vector <char *> cf;
	stream *f = openfile(cfg, "r");
	if(!f) { conoutf("no cfg found for %s", cfg); return; }
	char buf[4000];

	while(f->getline(buf, 4000))
	{
		char *pos = NULL;
		defformatstring(line)("%s", buf);
		loopk(2)
		{
			const char *part = !k ? a.part1 : a.part2;
			if(!part) continue;
			pos = strstr(buf, part); // go to .."lower_r.png> ..."
			if(pos && strstr(buf, a.ext)) // find our line: look for the model-file + the extension
			{
				char *rest = strstr(pos, "."); // go to the next dot (just don't put them into the filenames), otherwise everything's fine ".png> ..."
				if(!rest) break;
				*pos = '\0';
				formatstring(line)("%s%s_q_%s%s", buf, part, a.versions[version], rest);
			}
		}
		cf.add(newstring(line));
	}
	delete f;
	stream *w = openfile(path(cfg), "w");
	if(!w) { conoutf("can not rewrite cfg (%s)", cfg); return; }
	loopv(cf) f->printf(cf[i]);

	// cleanup
	delete w;
	loopv(cf) DELETEA(cf[i])
	cf.setsize(0);
}
...
void findqskins()
{
	...
	loopk(SKIN_MAX)
	{
		skininfo &a = skininfos[k];
		if(!a.part1 && !a.part2) continue;
		loopv(a.versions) DELETEA(a.versions[i])
		a.versions.setsize(0);
		vector<char *> files;
		formatstring(folder)("packages/models/%s/", a.folder);
		listfiles(folder, a.ext, files);
		...
		loopv(files)
		{
            ...
            formatstring(n)("%s%s", a.part1, SKINMARKER); // does not recognize the file when just the second part is available, but thats not really a bug, you just have to be complete

            if(strstr(file, n))
            {
                if(strlen(file) > strlen(n) && file[strlen(n)])
                {
                    char *v = a.versions.add(new string);
                    copystring(v, &file[strlen(n)]);
                }
            }
		}
	}
...
void setqskin(int model, int version)
{
	if(model < 0 || model >= SKIN_MAX) return;
	skininfo &a = skininfos[model];
	if(!a.versions.inrange(version)) return;
	char *v = a.versions[version];
	rewritemodelcfg(model, version);
	enumerate(textures, Texture, tex,
    {
		bool ispart1 = strstr(tex.name, a.part1) != 0;
		int texlen = strlen(tex.name);
		char *ending = &tex.name[texlen-3];
		bool correctending = !strcmp(a.ext, ending) || (!strcmp(a.ext, "dds") && !strcmp(ending, "png"));

		if(correctending && (ispart1 || (a.part2 && strstr(tex.name, a.part2))))
		{
			tex.name[texlen-4] = '\0';
			char *sp = strstr(tex.name, "_q_");
			if(sp) sp[0] = '\0';
			defformatstring(newt) ("%s_q_%s.%s", tex.name, v, ending);
			delete[] tex.name;
			tex.name = newstring(newt);
			ImageData s;
			int compress = 0;
			texturedata(s, tex.name, NULL, false, &compress);
			newtexture(&tex, NULL, s, tex.clamp, tex.mipmap, false, false, compress);
		}
	});

}
ICOMMAND(setqskin, "ii", (int *model, int *version), setqskin(*model, *version));

void loopqskins(int model)
{
	if(model < 0 || model >= SKIN_MAX) return;
	vector<char> buf;
	string version;
	int nums = 0;
	skininfo &a = skininfos[model];
	loopv(a.versions) if(a.versions[i])
	{
		formatstring(version)("%s", a.versions[i]);
		if(nums++) buf.add(' ');
		buf.put(version, strlen(version));
	}
	buf.add('\0');
	result(buf.getbuf());
}
ICOMMAND(loopqskins, "i", (int *model), loopqskins(*model));
...
QVARP(screenshotformat, "0: .bmp, 1: .tga, 2: .png, 3: .jpg", 0, IMG_JPG, NUMIMG-1);
...
QVARP(jpgquality, "quality of jpg-screenshots", 0, 80, 100);
QVARP(jpgoptimize, "optimize jpg-screenshots?", 0, 1, 1);
QVARP(jpgsubsampling, "subsampling of jpg-screenshots", 0, jpge::H2V2, jpge::H2V2);
...
void saveimage(const char *filename, int format, ImageData &image, bool flip = false)
{
    switch(format)
    {
        ...
        {
            jpge::params params;
            params.m_quality = jpgquality;
            params.m_subsampling = static_cast<jpge::subsampling_t>(jpgsubsampling);
            params.m_two_pass_flag = (jpgoptimize != 0);
            ImageData flipped(image.w, image.h, image.bpp, image.data);
            texflip(flipped);
            jpge::compress_image_to_jpeg_file(filename, flipped.w, flipped.h, 3, flipped.data, params);
            break;
        }
        default:
        {
...
namespace server
{
    extern const char *modename(int n, const char *unkown = "unkown");
    extern const char *mastermodename(int n, const char *unkown = "unkown");
}
...
void screenshot(char *filename, bool justfilename, int dir)
{
    static string buf;
    int format = -1;
    ...
    if(dir)
    {
        copystring(buf, tempformatstring("%s", dir == DIR_DUEL ? "duel/" : "clanwars/"));
        const char *dir = findfile(buf, "w");
        if(!fileexists(dir, "w")) createdir(dir);
    }
    if(filename[0] && justfilename)
    {
	...
	else
	{
		...
		if(isconnected(false, true))
            formatstring(name)("%d_%s_%s%d_%s%d.%s_%s_%s", timeinfo->tm_mday, months[timeinfo->tm_mon], timeinfo->tm_hour < 10 ? "0" : "", timeinfo->tm_hour, timeinfo->tm_min < 10 ? "0" : "",  timeinfo->tm_min, server::modename(game::gamemode), game::getclientmap(), justfilename ? "" : filename);
		else formatstring(name)("%d_%s_%s%d_%s%d", timeinfo->tm_mday, months[timeinfo->tm_mon], timeinfo->tm_hour < 10 ? "0" : "", timeinfo->tm_hour, timeinfo->tm_min < 10 ? "0" : "",  timeinfo->tm_min);
	...
	const char *foundfile = findfile(buf, "r");
	int d = 2;
	while(fileexists(foundfile, "r"))
	{
		string newname;
		int newlen = strlen(buf)-strlen(imageexts[format]) - (d > 2 ? size_t(4) : 0) - (d > 9 ? 1 : 0);
		strncpy(newname, buf, newlen);
		newname[newlen] = '\0';
		defformatstring(newending)(" (%d)%s%c", d, imageexts[format], '\0');
		concatstring(newname, newending);
		strcpy(buf, newname);
		foundfile = findfile(buf, "r");
		d++;
	}
